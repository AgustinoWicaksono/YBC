	...
	function f_tcalParseDate (s_date) {
		...
		s_date = arr_date[2]+'-'+arr_date[1]+'-'+arr_date[0];
		var re_date = /^\s*(\d{1,2})\-(\d{1,2})\-(\d{2,4})\s*$/;
		//var re_date = /^\s*(\d{1,2})\s*$/-(\d{1,2})\-(\d{2,4})\;
		...
		if (arr_date.length != 3) return alert ("Invalid date format: '" + s_date + "'.\nFormat accepted is dd-mm-yyyy.");
		if (!arr_date[2]) return alert ("Invalid date format: '" + s_date + "'.\nNo day of month value can be found.");
		if (!RE_NUM.exec(arr_date[2])) return alert ("Invalid day of month value: '" + arr_date[2] + "'.\nAllowed values are unsigned integers.");
		if (!arr_date[1]) return alert ("Invalid date format: '" + s_date + "'.\nNo month value can be found.");
		if (!RE_NUM.exec(arr_date[1])) return alert ("Invalid month value: '" + arr_date[1] + "'.\nAllowed values are unsigned integers.");
		if (!arr_date[0]) return alert ("Invalid date format: '" + s_date + "'.\nNo year value can be found.");
