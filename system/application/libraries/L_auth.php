		...
		if ($return['username_right']){
			...
			if(!empty($password)) { // empty password used when user forgot the password
				...
				if($return['password_right']) {
					...
					$employee = $this->obj->m_admin->check_employee_data($return['admin_id']);
			...
			$return['username_right'] = $this->obj->m_admin->check_employee_exist($username);

			if ($return['username_right']){

				if(!empty($password)) { // empty password used when user forgot the password
					$return = $this->obj->m_admin->check_employee_password($username, $password);
					...
					if($return['password_right']) {
						...
						// jika TIDAK ada login di tabel d_admin
						
							$plant = $this->obj->m_general->sap_plant_select_by_id($return['plant']);
...
// print_r($perm_groups_id); echo '<br />'.$admin_id.'<hr />';
