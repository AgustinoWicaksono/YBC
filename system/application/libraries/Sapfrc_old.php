...
//	function L_saprfc($config=array()) {
	function L_saprfc() {
	...
	function getStatusTextLong() {
		...
		switch ($this->status) {
			case SAPRFC_OK:
				...
				$statustext.="<br><b>".$this->obj->getStatusText()."</b>";
				$statustext.="</pre></font>";
				break;
			case SAPRFC_APPL_ERROR:
	...
	function printStatus() {
		echo $this->obj->getStatusTextLong();
	}		
	...
	function callFunction($func_name="",$parameters) {
		...
		*/
		...
		if (! extension_loaded ("saprfc")) {
			return $this->obj->setStatus(SAPRFC_ERROR,"saprfc::callFunction()\n SAPRFC-Extension.dll not loaded.");
		}
		...
		if ($this->obj->login()==SAPRFC_ERROR) {
		...
		foreach ($func_params_import as $name => $value) {
			...
			if (empty($rc)) {
				return $this->obj->setStatus(SAPRFC_ERROR,"saprfc::callFunction('".$func_name."')\n Import-Parameter=".$name. " could not be set. (Does it exist?)");
			}
		}
...
//			if (empty($rc)) { // llaegner removed (Reason: when export returns the -good- value "0", then empty also evaluates to true (thanks Alexander Gouriev)
		...
		if ($this->check_bapi_errors) {
			...
			if (isset($bapi_return) && 
				...
				$this->obj->setStatus(SAPRFC_APPL_ERROR,$bapi_return);
				return $result_data;	
			}
		}
		...
		// FINISH FUNCTION-CALL
		$this->obj->setStatus(SAPRFC_OK,"call function '".$func_name."' successfull.");
		return $result_data;	
	}
	
}

/* End of file L_saprfc.php */
/* Location: ./system/application/libraries/L_saprfc.php */