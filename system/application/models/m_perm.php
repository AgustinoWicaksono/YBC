...
class M_perm extends Model {
	 ...
	 * @param array $data array which contains data of fields which will be inserted to the table
	 * @return integer group_id field from d_perm_group
	 */
	 ...
	 * @param array $data array which contains data of fields which will be updated to the table
	 * @return bool
	 */
	...
	/**
	 * Get list of admin which have selected group permission
	...
	/**
	 * Update groups id (array, multiple) of a member to database
	 * @param integer $member_id
	 * @param string $member_perm_group_ids Contains permission group IDs, which contain data like "5, 7, 2, "
	 * @return bool
	 */
//	function member_perm_group_ids_update($member_id = 0, $data) {
//
//		if($member_id === 0)
//			$member_id = $this->obj->session->userdata['ADMIN']['admin_id'];
//
//		$this->db->where('member_id', $member_id);
//		if($this->db->update('d_member', $data))
//			return TRUE;
//		else
//			return FALSE;
//
...
//	function perm_groups_id_select($member_id = 0) {
//
//		if($member_id === 0)
//			$member_id = $this->obj->session->userdata['ADMIN']['admin_id'];
//
...
//				$perm_groups_id[] = $group['perm_grup_id'];
//			}
//			return $perm_groups_id;
//		} else {
//			return FALSE;
//		}
//	}
	...
	/**
	 * Get group permission names of an admin
	 * @param integer $admin_id
	 * @return string group permission names
	 */
...
//			die("<hr>");
			...
			foreach ($plants as $plant) {
				$outlet = $this->obj->m_general->sap_plant_select_by_id($plant);
...
//			die("<hr>");
			...
			foreach ($plant_permitted as $plant) {
				$outlet = $this->obj->m_general->sap_plant_select_by_id_summary($plant);
				if ($outlet!=FALSE) {
...
/* End of file m_perm.php */
/* Location: ./system/application/models/m_perm.php */