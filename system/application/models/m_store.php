...
class M_store extends Model {
	...
	public function __construct()
 	{
  		parent::__construct();
        $this->db2 = $this->load->database('MSI', TRUE);
		$this->load->library('msdb');
	}

	// start of browse

	function store_get_period_selection() {
	...
	function store_select_matr($date_from) {
       /* $plant = $this->session->userdata['ADMIN']['plant'];
...
//echo $swbQuery;
//echo $swbQuery;
//$this->db2->query("Usp_Report_store '$plant','$date_from'");
//$this->db2->from('rpt_store');
//$this->db2->where('WhsCode',$plant);*/
//$result = $this->msdb->output('Usp_Report_store', array('Whs'=>$plant, 'Date'=>$date_from), 'EXECUTE');
//$SP = "EXEC Usp_Report_store ?,?";
