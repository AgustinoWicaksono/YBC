...
function swbsendmail($to1, $toname1, $to2, $toname2, $cc1, $ccname1, $cc2, $ccname2, $from, $fromname, $mailsubject, $msg){
	...
	if (strtolower(@$to1)==strtolower(@$to2)) { $to2 = null; unset($to2); }
	if (strtolower(@$to1)==strtolower(@$cc1)) { $cc1 = null; unset($cc1); }
	if (strtolower(@$to1)==strtolower(@$cc2)) { $cc2 = null; unset($cc2); }

	if (strtolower(@$to2)==strtolower(@$to1)) { $to2 = null; unset($to2); }
	if (strtolower(@$to2)==strtolower(@$cc1)) { $cc1 = null; unset($cc1); }
	if (strtolower(@$to2)==strtolower(@$cc2)) { $cc2 = null; unset($cc2); }

	if ((Empty($to1))||($to1=="")) {$to1 = null;unset($to1);}
	if ((Empty($cc1))||($cc1=="")) {$cc1 = null;unset($cc1);}
	if ((Empty($to2))||($to2=="")) {$to2 = null;unset($to2);}
	if ((Empty($cc2))||($cc2=="")) {$cc2 = null;unset($cc2);}
...
*/
	...
	$swbAlt = str_ireplace("<br />","\r\n",$msg);
	$swbAlt = str_ireplace("<hr />","\r\n",$swbAlt);
	$mail->MsgHTML($msg);
	$mail->AltBody    = strip_tags($swbAlt);
...
//	$mail->Mailer = "smtp";
	...
	if (!Empty($cc1)) {
		if (strlen($cc1)>3)
			$mail->AddCC($cc1, $ccname1);
	}		
	
	if (!Empty($cc2)) {
		if (strlen($cc2)>3)
			$mail->AddCC($cc2, $ccname2);
...
//	$mail->Body = $msg;
	...
	if(!$mail->Send()) {
		$sendmailstatus = false;
		$sendmailinfo = @$mail->ErrorInfo;
		//echo "Email-Failed: ".$sendmailinfo;
	} else {
		$sendmailstatus = true;
		//echo "Email-Success";
	}
