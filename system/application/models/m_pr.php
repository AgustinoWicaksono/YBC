...
class M_pr extends Model {
	...
	function pr_headers_select_all() {
		$this->db->from('t_prnew_header');
		$this->db->order_by('id_pr_header');
	...
	/**
	 * Get how many result from a pr header.
	 * @return integer|false Count of result from a pr header.
	 */
	function pr_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
...
//echo $this->db->last_query();
	...
	function pr_headers_count_by_criteria_sap($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $back = 0, $sort = '') {
...
//echo $this->db->last_query();
	...
	function pr_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function pr_header_delete($id_pr_header) {

		if($this->pr_details_delete($id_pr_header)) {
	...
	function sap_pr_detail_select_by_id_pr_h_detail($id_pr_h_detail) {

		$this->db->from('wiwid_s_pr_detail');
		$this->db->where('id_pr_h_detail', $id_pr_h_detail);
	...
	function sap_pr_detail_select_by_item($item) {
	...
	function pr_select_to_export($id_pr_header) {
		$this->db->from('v_pr_export');
		$this->db->where_in('id_pr_header', $id_pr_header);
	...
	function sap_pr_header_approve($data) {
//echo "<pre>";
//print_r($data);
//echo "</pre>";
//exit;
	...
	function sap_pr_header_change($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       for ($i=1;$i<=$count;$i++) {
         if (!empty($data['material_no'][$i])) {
           $this->m_saprfc->append ("I_PR_ITEM",
              array ("EBELP"=>$data['id_pr_pr_line_no'][$i],
	...
	function sap_pr_header_cancel($data) {
//echo "<pre>";
//print '$data';
//print_r($data);
//echo "</pre>";
//exit;
	...
	function sap_pr_detail_approve($data) {
	...
	function pr_header_select($id_pr_header) {
		$this->db->from('t_prnew_header');
		$this->db->where('id_pr_header', $id_pr_header);
	...
	function pr_detail_update($data) {
	...
	function user_delete($pr_delete) {
		if($this->db->insert('t_delete', $pr_delete))
			return $this->db->insert_id();
		else
			return FALSE;
	}

	function pr_detail_update_by_id_h_detail($data) {
		$this->db->where('id_pr_h_detail', $data['id_pr_h_detail']);
		$this->db->where('id_pr_header', $data['id_pr_header']);
		if($this->db->update('t_pr_detail', $data))
			return TRUE;
		else
	...
	function pr_detail_delete($id_pr_detail) {
