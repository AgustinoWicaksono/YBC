...
class M_gitcc extends Model {
	...
	function gitcc_headers_select_all() {
		$this->db->from('t_gitcc_header');
		$this->db->order_by('id_gitcc_header');
	...
	/**
	 * Get how many result from a gitcc header.
	 * @return integer|false Count of result from a gitcc header.
	 */
	function gitcc_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
	...
	$date_from2 = $date_from.' 00:00:00';
		...
		if(!empty($sort)) {
			switch($sort) {
				case 'ay':
					$field_sort_name = 'id_gitcc_header';
					$field_sort_type = 'asc';
					break;
				case 'az':
					$field_sort_name = 'id_gitcc_header';
...
//echo $this->db->last_query();
	...
	function gitcc_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function gitcc_header_delete($id_gitcc_header) {

		if($this->gitcc_details_delete($id_gitcc_header)) {

			$this->db->where('id_gitcc_header', $id_gitcc_header);

			if($this->db->delete('t_gitcc_header'))
				return TRUE;
			else
				return FALSE;
	...
	function id_gitcc_plant_new_select($id_outlet,$posting_date="",$id_gitcc_header="") {
		...
		$this->db->select_max('id_gitcc_plant');
		$this->db->from('t_gitcc_header');
		$this->db->where('plant', $id_outlet);
		$this->db->where('DATE(posting_date)', $posting_date);
        if (!empty($id_gitcc_header)) {
    	  $this->db->where('id_gitcc_header <> ', $id_gitcc_header);
        }
		...
		if($query->num_rows() > 0) {
			$gitcc = $query->row_array();
			$id_gitcc_outlet = $gitcc['id_gitcc_plant'] + 1;
		}	else {
			$id_gitcc_outlet = 1;
		}

		return $id_gitcc_outlet;
	}

	function gitcc_header_insert($data) {
		if($this->db->insert('t_gitcc_header', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}

	function sap_gitcc_header_approve($data) {

       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       $this->m_saprfc->functionDiscover("ZMM_BAPI_CREATE_GI_TO_CC");
	...
	function sap_gitcc_header_cancel($data) {
//echo "<pre>";
//print_r($data);
//echo "</pre>";
//exit;
       ...
       if (empty($GOODSMVT_HEADRET['MAT_DOC'])) {
         ...
         $sap_messages = $sap_messages.
           ...
           'MATERIALDOCUMENT : '.$data['gitcc_no'].' <br> '.
           'OUTLET : '.$data['plant'].' <br> '.
           'POSTING_DATE : '.$data['posting_date'].' <br> '.
           'WEB_LOGIN_ID : '.$data['id_user_input'].' <br> '.
           'WEB_TRANSID : '.$data['web_trans_id'].' <br> ';
	...
	function gitcc_header_update($data) {
		$this->db->where('id_gitcc_header', $data['id_gitcc_header']);
		if($this->db->update('t_gitcc_header', $data))
			return TRUE;
		else
			return FALSE;
	}

	function gitcc_detail_insert($data) {
		if($this->db->insert('t_gitcc_detail', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	...
	function gitcc_is_data_cancelled($id_gitcc_header) {
		$this->db->from('t_gitcc_detail');
		$this->db->where('id_gitcc_header', $id_gitcc_header);
	...
	function gitcc_header_select($id_gitcc_header) {
		$this->db->from('t_gitcc_header');
		$this->db->where('id_gitcc_header', $id_gitcc_header);
	...
	function gitcc_details_select($id_gitcc_header) {
		$this->db->from('t_gitcc_detail');
		$this->db->where('id_gitcc_header', $id_gitcc_header);
		$this->db->order_by('id_gitcc_detail');
	...
	function gitcc_detail_update($data) {
		$this->db->where('id_gitcc_detail', $data['id_gitcc_detail']);
		if($this->db->update('t_gitcc_detail', $data))
			return TRUE;
		else
			return FALSE;
	}

	function gitcc_details_delete($id_gitcc_header) {
		$this->db->where('id_gitcc_header', $id_gitcc_header);
		if($this->db->delete('t_gitcc_detail'))
			return TRUE;
		else
			return FALSE;
	}
