...
class M_produksi extends Model {
	...
	function produksi_headers_select_all() {
		$this->db->from('t_produksi_header');
		$this->db->order_by('id_produksi_header');
	...
	/**
	 * Get how many result from a produksi header.
	 * @return integer|false Count of result from a produksi header.
	 */
	function produksi_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
		...
		if(!empty($field_content)) {
			...
			switch($field_name) {
				case 'a':
					...
					break;
				case 'b':
					$field_name_ori = 'produksi_no';
					break;
				case 'c':
					$field_name_ori = 'doc_issue';
					break;
				case 'd':
	...
	$date_from2 = $date_from.' 00:00:00';
		...
		if(!empty($sort)) {
			switch($sort) {
				case 'ay':
					$field_sort_name = 'id_produksi_header';
					$field_sort_type = 'asc';
					break;
				case 'az':
					$field_sort_name = 'id_produksi_header';
...
//echo $this->db->last_query();
	...
	function produksi_headers_count_by_criteria_sap($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0 , $back = 0, $sort = '') {
...
//echo $this->db->last_query();
	...
	function produksi_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function produksi_header_delete($id_produksi_header) {

		if($this->produksi_details_delete($id_produksi_header)) {
	...
	function sap_produksi_detail_select_by_id_produksi_h_detail($id_produksi_h_detail) {

		$this->db->from('wiwid_s_produksi_detail');
		$this->db->where('id_produksi_h_detail', $id_produksi_h_detail);
	...
	function sap_produksi_detail_select_by_item($item) {
	...
	function sap_produksi_header_approve($data) {
	...
	function sap_produksi_header_cancel($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       if (empty($GOODSMVT_HEADRET['MAT_DOC'])) {
         ...
         $sap_messages = $sap_messages.
           ...
           'MATERIALDOCUMENT : '.$data['produksi_no'].' <br> '.
           'OUTLET : '.$data['plant'].' <br> '.
           'POSTING_DATE : '.$data['posting_date'].' <br> '.
           'WEB_LOGIN_ID : '.$data['id_user_input'].' <br> '.
           'WEB_TRANSID : '.$data['web_trans_id'].' <br> ';
	...
	function produksi_header_update($data) {
		$this->db->where('id_produksi_header', $data['id_produksi_header']);
		if($this->db->update('t_produksi_header', $data))
			return TRUE;
		else
			return FALSE;
	}

	function produksi_header_update_by_produksi_no($data) {
		$this->db->where('produksi_no', $data['produksi_no']);
		if($this->db->update('t_produksi_header', $data))
			return TRUE;
		else
			return FALSE;
	...
	function sap_produksi_detail_approve($data) {
	...
	function produksi_header_select($id_produksi_header) {
		$this->db->from('t_produksi_header');
		$this->db->where('id_produksi_header', $id_produksi_header);
	...
	function produksi_details_select($id_produksi_header) {
	...
	function produksi_is_data_cancelled($id_produksi_header) {
		$this->db->from('t_produksi_detail');
		$this->db->where('id_produksi_header', $id_produksi_header);
	...
	function produksi_detail_update($data) {
		$this->db->where('id_produksi_detail', $data['id_produksi_detail']);
		if($this->db->update('t_produksi_detail', $data))
			return TRUE;
		else
			return FALSE;
	}
	...
	function produksi_details_delete($id_produksi_header) {
		$this->db->where('id_produksi_header', $id_produksi_header);
	...
	function produksi_temp_delete($item,$whs1,$user1) {
		$this->db->where('material_no', $item);
		$this->db->where('whs', $whs1);
	...
	function user_delete($produksi_delete) {
		if($this->db->insert('t_delete', $produksi_delete))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	
	function batch_delete($id_produksi_header) {
		$this->db->where('BaseEntry', $id_produksi_header);
