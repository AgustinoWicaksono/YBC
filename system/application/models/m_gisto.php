...
class M_gisto extends Model {
	...
	function gisto_headers_select_all() {
		$this->Fdb->from('t_gisto_header');
		$this->db->order_by('id_gisto_header');
	...
	/**
	 * Get how many result from a gisto header.
	 * @return integer|false Count of result from a gisto header.
	 */
	function gisto_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
	...
	$date_from2 = $date_from.' 00:00:00';
		...
		if(!empty($sort)) {
			switch($sort) {
				case 'ay':
					$field_sort_name = 'id_gisto_header';
					$field_sort_type = 'asc';
					break;
				case 'az':
					$field_sort_name = 'id_gisto_header';
...
//echo $this->db->last_query();
	...
	function gisto_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function gisto_header_delete($id_gisto_header) {

		if($this->gisto_details_delete($id_gisto_header)) {

			$this->db->where('id_gisto_header', $id_gisto_header);

			if($this->db->delete('t_gisto_header'))
				return TRUE;
			else
				return FALSE;
	...
	function sap_gisto_detail_select_by_id_gisto_h_detail($id_gisto_h_detail) {

		$this->db->from('wiwid_s_gisto_detail');
		$this->db->where('id_gisto_h_detail', $id_gisto_h_detail);
	...
	function sap_gisto_detail_select_by_item($item) {
	...
	function id_gisto_plant_new_select($id_outlet,$posting_date="",$id_gisto_header="") {
		...
		$this->db->select_max('id_gisto_plant');
		$this->db->from('t_gisto_header');
		$this->db->where('plant', $id_outlet);
		$this->db->where('DATE(posting_date)', $posting_date);
        if (!empty($id_gisto_header)) {
    	  $this->db->where('id_gisto_header <> ', $id_gisto_header);
        }
		...
		if($query->num_rows() > 0) {
			$gisto = $query->row_array();
			$id_gisto_outlet = $gisto['id_gisto_plant'] + 1;
		}	else {
			$id_gisto_outlet = 1;
		}

		return $id_gisto_outlet;
	}

	function gisto_header_insert($data) {
		if($this->db->insert('t_gisto_header', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}

	function sap_gisto_header_approve($data) {
	...
	function sap_gisto_header_cancel($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       if (empty($GOODSMVT_HEADRET['MAT_DOC'])) {
         ...
         $sap_messages = $sap_messages.
           ...
           'MATERIALDOCUMENT : '.$data['gisto_no'].' <br> '.
           'OUTLET : '.$data['plant'].' <br> '.
           'POSTING_DATE : '.$data['posting_date'].' <br> '.
           'WEB_LOGIN_ID : '.$data['id_user_input'].' <br> '.
           'WEB_TRANSID : '.$data['web_trans_id'].' <br> ';
	...
	function gisto_header_update($data) {
		$this->db->where('id_gisto_header', $data['id_gisto_header']);
		if($this->db->update('t_gisto_header', $data))
			return TRUE;
		else
			return FALSE;
	}

	function gisto_header_update_by_gisto_no($data) {
		$this->db->where('gisto_no', $data['gisto_no']);
		if($this->db->update('t_gisto_header', $data))
			return TRUE;
		else
			return FALSE;
	...
	function gisto_detail_insert($data) {
		if($this->db->insert('t_gisto_detail', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	...
	function sap_gisto_detail_approve($data) {
	...
	function gisto_header_select($id_gisto_header) {
		$this->db->from('t_gisto_header');
		$this->db->where('id_gisto_header', $id_gisto_header);
	...
	function gisto_details_select($id_gisto_header) {
		$this->db->from('t_gisto_detail');
		$this->db->where('id_gisto_header', $id_gisto_header);
		$this->db->order_by('id_gisto_detail');
	...
	function gisto_is_data_cancelled($id_gisto_header) {
		$this->db->from('t_gisto_detail');
		$this->db->where('id_gisto_header', $id_gisto_header);
	...
	function gisto_detail_update($data) {
		$this->db->where('id_gisto_detail', $data['id_gisto_detail']);
		if($this->db->update('t_gisto_detail', $data))
			return TRUE;
		else
			return FALSE;
	}
	...
	function gisto_details_delete($id_gisto_header) {
		$this->db->where('id_gisto_header', $id_gisto_header);
		if($this->db->delete('t_gisto_detail'))
			return TRUE;
		else
			return FALSE;
	}
