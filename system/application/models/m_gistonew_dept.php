<?php
class M_gistonew_dept extends Model {
...
public function __construct()
	...
	function gistonew_dept_headers_select_all() {
		$this->db->from('t_gistonew_dept_header');
		$this->db->order_by('id_gistonew_dept_header');
	...
	function user_delete($gistonew_dept_delete) {
		if($this->db->insert('t_delete', $gistonew_dept_delete))
...
//        return "Data per <b>".date("d M Y - H:i:s",strtotime($last_posto_update))." WIB</b>";
	...
	public function sap_do_select_all($kd_plant="",$do_no="",$do_item="") {
		...
		$c=mssql_connect("SVR-S-2012-64","sa","M$1S@p!#@");
		...
		//{
		 ...
		 $this->db2->select('OWTQ.DocEntry VBELN,OWTQ.DocDate DELIV_DATE,OWTQ.ToWhsCode,WTQ1.LineNum POSNR,
	...
	function sap_gistonew_dept_header_select_by_do_no($do_no) {
	...
	function sap_gistonew_dept_select_item_group_do($do_no) {
        $doitems = $this->sap_gistonew_dept_details_select_by_do_no($do_no);
...
function sap_gistonew_dept_details_select_by_do_no($do_no) {
        if (empty($this->session->userdata['do_nos'])) {
        ...
        if ($count > 0) {
          for($i=1;$i<=$count;$i++) {
            $doitems[$i]['id_gistonew_dept_h_detail'] = $i;
          }
          return $doitems;
        }
        else {
...
function sap_gistonew_dept_details_select_by_do_and_item_group($do_no,$item_group_code) {
        $doitems = $this->sap_gistonew_dept_details_select_by_do_no($do_no);
        $count = count($doitems);
        $k = 1;
        for ($i=1;$i<=$count;$i++) {
	...
	function sap_gistonew_dept_details_select_by_do_and_item_code($do_no,$item_code) {
        $doitems = $this->sap_gistonew_dept_details_select_by_do_no($do_no);
        $count = count($doitems);
        $k = 1;
        for ($i=1;$i<=$count;$i++) {
	...
	/**
	 * Get how many result from a gistonew_dept header.
	 * @return integer|false Count of result from a gistonew_dept header.
	 */
	function gistonew_dept_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
		...
		if(!empty($field_content)) {
			...
			switch($field_name) {
				case 'a':
					$field_name_ori = 'gistonew_dept_no';
					break;
				case 'b':
	...
	$date_from2 = $date_from.' 00:00:00';
		...
		if(!empty($sort)) {
			switch($sort) {
				case 'ay':
					$field_sort_name = 'id_gistonew_dept_header';
					$field_sort_type = 'asc';
					break;
				case 'az':
					$field_sort_name = 'id_gistonew_dept_header';
					...
					break;
				case 'by':
					$field_sort_name = 'gistonew_dept_no';
					$field_sort_type = 'asc';
					break;
				case 'bz':
					$field_sort_name = 'gistonew_dept_no';
...
//echo $this->db->last_query();
	...
	function gistonew_dept_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function gistonew_dept_header_delete($id_gistonew_dept_header) {

		if($this->gistonew_dept_details_delete($id_gistonew_dept_header)) {

			$this->db->where('id_gistonew_dept_header', $id_gistonew_dept_header);

			if($this->db->delete('t_gistonew_dept_header'))
				return TRUE;
			else
				return FALSE;
	...
	function sap_gistonew_dept_detail_select_by_id_gistonew_dept_h_detail($id_gistonew_dept_h_detail) {

		$this->db->from('wiwid_s_gistonew_dept_detail');
		$this->db->where('id_gistonew_dept_h_detail', $id_gistonew_dept_h_detail);
	...
	function sap_gistonew_dept_detail_select_by_item($item) {
	...
	function id_gistonew_dept_plant_new_select($id_outlet,$posting_date="",$id_gistonew_dept_header="") {
		...
		$this->db->select_max('id_gistonew_dept_plant');
		$this->db->from('t_gistonew_dept_header');
		$this->db->where('plant', $id_outlet);
		$this->db->where('DATE(posting_date)', $posting_date);
        if (!empty($id_gistonew_dept_header)) {
    	  $this->db->where('id_gistonew_dept_header <> ', $id_gistonew_dept_header);
        }
		...
		if($query->num_rows() > 0) {
			$gistonew_dept = $query->row_array();
			$id_gistonew_dept_outlet = $gistonew_dept['id_gistonew_dept_plant'] + 1;
		}	else {
			$id_gistonew_dept_outlet = 1;
		}

		return $id_gistonew_dept_outlet;
	}

	function gistonew_dept_header_insert($data) {
	//echo 'xxxxx';
		if($this->db->insert('t_gistonew_dept_header', $data))
	...
	function sap_gistonew_dept_header_approve($data) {
	...
	function sap_gistonew_dept_header_cancel($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       if (empty($GOODSMVT_HEADRET['MAT_DOC'])) {
         ...
         $sap_messages = $sap_messages.
           ...
           'MATERIALDOCUMENT : '.$data['gistonew_dept_no'].' <br> '.
           'OUTLET : '.$data['plant'].' <br> '.
           'POSTING_DATE : '.$data['posting_date'].' <br> '.
           'WEB_LOGIN_ID : '.$data['id_user_input'].' <br> '.
           'WEB_TRANSID : '.$data['web_trans_id'].' <br> ';
	...
	function gistonew_dept_header_update($data) {
		$this->db->where('id_gistonew_dept_header', $data['id_gistonew_dept_header']);
		if($this->db->update('t_gistonew_dept_header', $data))
			return TRUE;
		else
			return FALSE;
	}

	function gistonew_dept_header_update_by_gistonew_dept_no($data) {
		$this->db->where('gistonew_dept_no', $data['gistonew_dept_no']);
		if($this->db->update('t_gistonew_dept_header', $data))
			return TRUE;
		else
			return FALSE;
	...
	function gistonew_dept_detail_insert($data) {
		if($this->db->insert('t_gistonew_dept_detail', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	...
	function sap_gistonew_dept_detail_approve($data) {
	...
	function gistonew_dept_header_select($id_gistonew_dept_header) {
		$this->db->from('t_gistonew_dept_header');
		$this->db->where('id_gistonew_dept_header', $id_gistonew_dept_header);
	...
	function gistonew_dept_details_select($id_gistonew_dept_header) {
		$this->db->from('t_gistonew_dept_detail');
		$this->db->where('id_gistonew_dept_header', $id_gistonew_dept_header);
		$this->db->order_by('id_gistonew_dept_detail');
	...
	function gistonew_dept_is_data_cancelled($id_gistonew_dept_header) {
		$this->db->from('t_gistonew_dept_detail');
		$this->db->where('id_gistonew_dept_header', $id_gistonew_dept_header);
	...
	function gistonew_dept_detail_update($data) {
		$this->db->where('id_gistonew_dept_detail', $data['id_gistonew_dept_detail']);
		if($this->db->update('t_gistonew_dept_detail', $data))
			return TRUE;
		else
			return FALSE;
	}
	...
	function gistonew_dept_details_delete($id_gistonew_dept_header) {
		$this->db->where('id_gistonew_dept_header', $id_gistonew_dept_header);
		if($this->db->delete('t_gistonew_dept_detail'))
			return TRUE;
	...
	function batch_delete($id_gistonew_dept_header) {
		$this->db->where('BaseEntry', $id_gistonew_dept_header);
