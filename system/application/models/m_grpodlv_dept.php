<?php
class M_grpodlv_dept extends Model {

	public function __construct()
 	{
  		parent::__construct();
        //$this->db2 = $this->load->database('Test_MSI', TRUE);
	}
	function M_grpodlv_dept() {
		parent::Model();
	...
	function grpodlv_dept_headers_select_all() {
		$this->db->from('t_grpodlv_dept_header');
		$this->db->order_by('id_grpodlv_dept_header');
	...
	function grpodlv_dept_headers_count_by_criteria1() {
		$this->db->from('t_grpodlv_dept_header');
	...
	/**
	 * Get how many result from a grpodlv_dept header.
	 * @return integer|false Count of result from a grpodlv_dept header.
	 */
	function grpodlv_dept_headers_count_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '') {
		...
		if(!empty($field_content)) {
			...
			switch($field_name) {
				case 'a':
					$field_name_ori = 'grpodlv_dept_no';
					break;
				case 'b':
	...
	$date_from2 = $date_from.' 00:00:00';
		...
		if(!empty($sort)) {
			switch($sort) {
				case 'ay':
					$field_sort_name = 'id_grpodlv_dept_header';
					$field_sort_type = 'asc';
					break;
				case 'az':
					$field_sort_name = 'id_grpodlv_dept_header';
				...
				case 'bz':
					...
					break;
				case 'cy':
					$field_sort_name = 'grpodlv_dept_no';
					$field_sort_type = 'asc';
					break;
				case 'cz':
					$field_sort_name = 'grpodlv_dept_no';
...
//echo $this->db->last_query();
	...
	function grpodlv_dept_headers_select_by_criteria($field_name = '', $field_type = '', $field_content = '', $date_from = '', $date_to = '', $status = 0, $sort = '', $limit = 10, $start = 0) {
...
//echo $this->db->last_query();
	...
	function grpodlv_dept_is_data_cancelled($id_grpodlv_dept_header) {
		$this->db->from('t_grpodlv_dept_detail');
		$this->db->where('id_grpodlv_dept_header', $id_grpodlv_dept_header);
	...
	function grpodlv_dept_header_delete($id_grpodlv_dept_header) {

		if($this->grpodlv_dept_details_delete($id_grpodlv_dept_header)) {

			$this->db->where('id_grpodlv_dept_header', $id_grpodlv_dept_header);

			if($this->db->delete('t_grpodlv_dept_header'))
				return TRUE;
			else
				return FALSE;
...
*/
	...
	function grpodlv_dept_select_to_export($id_grpodlv_dept_header) {
		$this->db->from('v_grpodlv_dept_export');
		$this->db->where_in('id_grpodlv_dept_header', $id_grpodlv_dept_header);
	...
	function sap_grpodlv_dept_header_select_by_do_no($do_no) {
	...
	function sap_grpodlv_dept_details_select_by_do_no($do_no) {
        if (empty($this->session->userdata['do_nos'])) {
        ...
        if ($count > 0) {
          for($i=1;$i<=$count;$i++) {
            $doitems[$i]['id_grpodlv_dept_h_detail'] = $i;
          }
          return $doitems;
        }
        else {
	...
	function sap_grpodlv_dept_details_select_by_do_and_item($do_no,$item) {
        if (empty($this->session->userdata['do_nos'])) {
	...
	function sap_grpodlv_dept_select_item_group_do($do_no) {
        $doitems = $this->sap_grpodlv_dept_details_select_by_do_no($do_no);
	...
	function sap_grpodlv_dept_details_select_by_do_and_item_group($do_no,$item_group_code) {
        $doitems = $this->sap_grpodlv_dept_details_select_by_do_no($do_no);
        $count = count($doitems);
        $k = 1;
        for ($i=1;$i<=$count;$i++) {
	...
	function sap_grpodlv_dept_details_select_by_do_and_item_code($do_no,$item_code) {
        $doitems = $this->sap_grpodlv_dept_details_select_by_do_no($do_no);
        $count = count($doitems);
        $k = 1;
        for ($i=1;$i<=$count;$i++) {
	...
	function sap_grpodlv_dept_detail_select_by_id_grpodlv_dept_h_detail($id_grpodlv_dept_h_detail) {

		$this->db->from('wiwid_s_grpodlv_dept_detail');
		$this->db->where('id_grpodlv_dept_h_detail', $id_grpodlv_dept_h_detail);
	...
	function id_grpodlv_dept_plant_new_select($id_outlet,$posting_date="",$id_grpodlv_dept_header="") {
		...
		$this->db->select_max('id_grpodlv_dept_plant');
		$this->db->from('t_grpodlv_dept_header');
		$this->db->where('plant', $id_outlet);
		$this->db->where('DATE(posting_date)', $posting_date);
        if (!empty($id_grpodlv_dept_header)) {
    	  $this->db->where('id_grpodlv_dept_header <> ', $id_grpodlv_dept_header);
        }
		...
		if($query->num_rows() > 0) {
			$grpodlv_dept = $query->row_array();
			$id_grpodlv_dept_outlet = $grpodlv_dept['id_grpodlv_dept_plant'] + 1;
		}	else {
			$id_grpodlv_dept_outlet = 1;
		}

		return $id_grpodlv_dept_outlet;
	}

    function grpodlv_dept_header_insert($data) {
		if($this->db->insert('t_grpodlv_dept_header', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}

	function sap_grpodlv_dept_header_approve($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       if($ERRORX=='X') {
        ...
        if($ERRORX=='X') {
          unset($approved_data['material_document']);
           ...
           $count = count($I_PO_STO);
           for($i=1;$i<=$count;$i++) {
             $this->grpodlv_dept_db_update($I_PO_STO[$i]['DELV_NMR'],$I_PO_STO[$i]['ITEM_NMR'],$I_PO_STO[$i]['GR_QNTY'],'+');
           }
	...
	function grpodlv_dept_db_update($vblen,$posnr,$qty,$oprd) {
        $kd_plant = $this->session->userdata['ADMIN']['plant'];
		$this->db->where('PLANT', $kd_plant);
	...
	function sap_grpodlv_dept_header_cancel($data) {
       $this->m_saprfc->setUserRfc();
       $this->m_saprfc->setPassRfc();
       $this->m_saprfc->sapAttr();
       $this->m_saprfc->connect();
       ...
       if (empty($GOODSMVT_HEADRET['MAT_DOC'])) {
         ...
         $sap_messages = $sap_messages.
           ...
           'MATERIALDOCUMENT : '.$data['grpodlv_dept_no'].' <br> '.
           'OUTLET : '.$data['plant'].' <br> '.
           'POSTING_DATE : '.$data['posting_date'].' <br> '.
           'WEB_LOGIN_ID : '.$data['id_user_input'].' <br> '.
           'WEB_TRANSID : '.$data['web_trans_id'].' <br> ';
         for($i=1;$i<=$count;$i++) {
           ...
           $count = count($MATDOC_ITEM);
           for($i=1;$i<=$count;$i++) {
             $this->grpodlv_dept_db_update($data['do_no'],$data['item'][$i],$data['gr_quantity'][$i],'-');
           }
	   }
	...
	function grpodlv_dept_header_update($data) {
		$this->db->where('id_grpodlv_dept_header', $data['id_grpodlv_dept_header']);
		if($this->db->update('t_grpodlv_dept_header', $data))
			return TRUE;
		else
			return FALSE;
	}

	function grpodlv_dept_detail_insert($data) {
		if($this->db->insert('t_grpodlv_dept_detail', $data))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	...
	function sap_grpodlv_dept_detail_approve($data) {
	...
	function grpodlv_dept_header_select($id_grpodlv_dept_header) {
		$this->db->from('t_grpodlv_dept_header');
		$this->db->where('id_grpodlv_dept_header', $id_grpodlv_dept_header);
	...
	function grpodlv_dept_details_select($id_grpodlv_dept_header) {
		$this->db->from('t_grpodlv_dept_detail');
		$this->db->where('id_grpodlv_dept_header', $id_grpodlv_dept_header);
		$this->db->order_by('id_grpodlv_dept_detail');
	...
	function grpodlv_dept_detail_update($data) {
		$this->db->where('id_grpodlv_dept_detail', $data['id_grpodlv_dept_detail']);
		if($this->db->update('t_grpodlv_dept_detail', $data))
			return TRUE;
		else
			return FALSE;
	}
	
	function user_delete($grpodlv_dept_delete) {
		if($this->db->insert('t_delete', $grpodlv_dept_delete))
			return $this->db->insert_id();
		else
			return FALSE;
	}
	...
	function grpodlv_dept_details_delete($id_grpodlv_dept_header) {
		$this->db->where('id_grpodlv_dept_header', $id_grpodlv_dept_header);
		if($this->db->delete('t_grpodlv_dept_detail'))
			return TRUE;
		else
			return FALSE;
	}

	function grpodlv_dept_detail_delete($id_grpodlv_dept_detail) {
		$this->db->where('id_grpodlv_dept_detail', $id_grpodlv_dept_detail);
		if($this->db->delete('t_grpodlv_dept_detail'))
			return TRUE;
		else
			return FALSE;
