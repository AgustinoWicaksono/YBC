...
class M_bincard extends Model {
	...
	function bincard_get_period_selection() {
	...
	function bincard_select_matr($date_from) {
        $plant = $this->session->userdata['ADMIN']['plant'];
...
//echo $swbQuery;
//echo $swbQuery;
//$this->db2->query("Usp_Report_bincard '$plant','$date_from'");
//$this->db2->from('rpt_bincard');
//$this->db2->where('WhsCode',$plant);*/
//$result = $this->msdb->output('Usp_Report_bincard', array('Whs'=>$plant, 'Date'=>$date_from), 'EXECUTE');
//$SP = "EXEC Usp_Report_bincard ?,?";
