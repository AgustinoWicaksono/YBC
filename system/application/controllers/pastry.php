...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_pastry'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_pastry->pastry_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Inventory Audit ';
		$this->template->write_view('content', 'pastry/pastry_browse', $object);
	...
	// search data
		...
		$date_to =$_POST['to'];
        $group2 = $_POST['item_group_code'];
        $group1 = $_POST['WhsCode'];

		redirect('pastry/browse_result/'.$date_from.'/'.$group1.'/'.$group2.'/'.$date_to);

	}
	
	function browse_search_print() {
        redirect('pastry/browse_result_excel/'.$date_from.'/'.$group1.'/'.$group2.'/'.$date_to);

	}

	// search result
	function browse_result($date_from = '', $group1 = '', $group2 = '',$date_to='')	{

		$this->l_page->save_page('pastry_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_pastry->pastry_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_pastry');
		//$data['data'] = $this->m_pastry->pastry_select_matr($date_from);
		$object['date_from']=$date_from;
		$object['date_to']=$date_to;
		$object['item_group_code']=$group2;
		$object['WhsCode']=$group1;
		
		//echo $object['date'];
	//	$object['data'] = $data;
		$object['page_title'] = 'Inventory Audit';

		//$this->load->view('pastry/pastry_browse_result', $object);
		$this->load->view('pastry/pastry_browse_result', $object);

	}
	
	function browse_result_excel($date_from, $group1, $group2, $date_to)	{

		$this->l_page->save_page('pastry_browse_result_excel');
	/*	if($date_from !== '0') {
			$year = substr($date_from, 0, 4);
			$month = substr($date_from, 4, 2);
			$day = substr($date_from, 6, 2);
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('pastry/pastry_browse_result', $object);
		$this->load->view('pastry/pastry_browse_result_excel', $object);
