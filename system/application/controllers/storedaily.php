<?php
class storedaily extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_bread'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Store Daily  ';
		$this->template->write_view('content', 'storedaily/storedaily_browse', $object);
	...
	function browse_search() {
		...
		redirect('storedaily/storedaily_result/'.$date_from.'/'.$group.'/');
	...
	function browse_result($date_from = '',$group='')	{
		...
		$this->l_page->save_page('storedaily_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_bread->bread_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_bread');
		//$data['data'] = $this->m_bread->bread_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('bread/bread_browse_result', $object);
		$this->load->view('storedaily/storedaily_browse_result', $object);
