...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_store'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_store->store_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Inventory Movement ';
		$this->template->write_view('content', 'store/store_browse', $object);
	...
	function browse_search() {
		...
		redirect('store/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '', $group = '',$date_to='')	{
		...
		$this->l_page->save_page('store_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_store->store_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_store');
		//$data['data'] = $this->m_store->store_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('store/store_browse_result', $object);
		$this->load->view('store/store_browse_result', $object);
