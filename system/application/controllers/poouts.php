<?php
class Poouts extends Controller {
...
//			$this->l_auth->save_referer_and_login();
	...
	function browse() {
		...
		if(!empty($agging_browse_result))
			...
			redirect('poouts/browse_result');
	
	}

	function browse_result()	{

		$this->l_page->save_page('poouts_browse_result');
		...
		$config['base_url'] = site_url('poouts/browse_result/'.$field_name.'/'.$field_type.'/'.$field_content.'/'.$sort.'/'.$limit.'/');

		$config['total_rows'] = $this->m_poouts->poouts_select_all_count();
		$this->pagination->initialize($config);
		$object['data']['poouts'] = $this->m_poouts->poouts_select_all();
		
		$object['limit'] = $limit;
		$object['total_rows'] = $config['total_rows'];


		$object['page_title'] = 'List PO Outstanding';
		$this->template->write_view('content', 'poouts/poouts_browse', $object);
	...
	function edit() {
        ...
        $data = $this->m_poouts->poouts_select($this->uri->segment(3));
  			$this->_data_form(0, $data);
      }
...
//		$this->form_validation->set_rules('id_agging', 'Employee Position Code', 'trim|required');
		$this->form_validation->set_rules('poouts', 'Employee Position', 'trim|required');
	}
...
//			$member['data']['membership_status'] = 2;
		...
		$this->template->write_view('content', 'poouts/agging_input', $object);
		$this->template->render();
	}
	...
	function _data_add() {
		...
		if($this->m_poouts->agging_add($poouts)) {

			$object['page_title'] = 'Data poouts Berhasil Dimasukkan';
			$object['refresh'] = 1;
			$object['refresh_text'] = 'Data poouts berhasil dimasukkan.';
	...
	function _data_update()	{
		...
		if($this->m_poouts->agging_update($poouts)) {
			
			$object['page_title'] = 'Data poouts Berhasil Diubah';
			$object['refresh'] = 1;
			$object['refresh_text'] = 'Data poouts berhasil diubah.';
			$object['refresh_url'] = $this->session->userdata['PAGE']['next'];
