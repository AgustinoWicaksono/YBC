<?php
class spoiled2 extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_spoiled'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Spoiled ';
		$this->template->write_view('content', 'spoiled2/spoiled2_browse', $object);
	...
	function browse_search() {
		...
		redirect('spoiled2/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='',$date_to = '')	{
		...
		$this->l_page->save_page('spoiled2_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		$this->load->view('spoiled2/spoiled2_browse_result', $object);
