...
;
	...
	function browse() {
		...
		if(!empty($admin_browse_result))
			...
			redirect('user/browse_result/0/0/0/0/10');
...
//		redirect('user/browse_result/'.$_POST['field_name']."/".$_POST['field_type']."/".$field_content."/".$date_from."/".$date_to."/".$_POST['status']."/".$_POST['sort1']."/".$_POST['sort2']."/".$_POST['sort3']."/".$limit);
		redirect('user/browse_result/'.$_POST['field_name']."/".$_POST['field_type']."/".$field_content."/0/".$limit);
...
*/
		...
		$sort_link1 = 'user/browse_result/'.$field_name.'/'.$field_type.'/'.$field_content.'/';
		$sort_link2 = '/'.$limit.'/';
		...
		$config['base_url'] = site_url('user/browse_result/'.$field_name.'/'.$field_type.'/'.$field_content.'/'.$sort.'/'.$limit.'/');

		$config['total_rows'] = $this->m_user->admin_count_by_criteria($field_name, $field_type, $field_content, $sort);;

		// save pagination definition
		$this->pagination->initialize($config);

		$object['data']['admins'] = $this->m_user->admin_select_by_criteria($field_name, $field_type, $field_content, $sort, $limit, $start);

		$object['limit'] = $limit;
		$object['total_rows'] = $config['total_rows'];

		$object['page_title'] = 'Managemen Pengguna';
		$this->template->write_view('content', 'user/admin_browse', $object);
	...
	function view($admin_id = 0) {
		...
		$object['data'] = $this->m_user->admin_select($admin_id);

		// save this page to referer
		$this->l_page->save_page('next');

		$this->template->write_view('content', 'user/admin_view', $object);
		$this->template->render();
	...
	function delete($admin_id) {
		...
		$this->m_user->admin_delete($admin_id);
...
// print_r($data['admin']['admin_id']);die();
		...
		if($object['plant_types'] !== FALSE) {
			...
			//hardcode
			$object['plant_type']['OFFICE'] = 'MSI Office';
			$hasil = $this->m_user->select_plants('OFFICE',$data['admin']['admin_id'],$object['data']['admin']['plants'],$object['plant_terpilih'],$object['plant_terpilih_count']);
			...
			foreach ($object['plant_types']->result_array() as $plant_type) {
				$object['plant_type'][$plant_type['id_jenisoutlet']] = $plant_type['jenisoutlet'];
				$hasil = $this->m_user->select_plants($plant_type['id_jenisoutlet'],$data['admin']['admin_id'],$object['data']['admin']['plants'],$object['plant_terpilih'],$object['plant_terpilih_count']);
				$object['plant_terpilih'] = $hasil['plant_terpilih'];
				$object['plant_terpilih_count'] = $hasil['plant_terpilih_count'];
...
//new			
		...
		$this->template->write_view('content', 'user/admin_profile_form', $object);
		$this->template->render();
	}
	...
	function _input_add() {
		...
		if (!$this->m_user->check_username_exist($admin['admin_username'])) {
			$data_nik = $_POST['data_nik'];
			if ($data_nik!='') {
				$admin['admin_emp_id'] = $this->m_user->select_employee_id($data_nik);
			} else {
			...
			if ($admin['plant']!='')
				$admin['plant_type_id'] = $this->m_user->select_company_code($admin['plant']);
	...
	function _input_success($admin_username) {
		...
		$object['refresh_url'] = site_url('user/browse_result/b/part/'.$admin_username.'/0/100');
	...
	function password_edit($admin_id = 0) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$data = $this->m_user->admin_select($admin_id);
				$this->_password_form(0, $data);
			}
...
//		$this->form_validation->set_rules('admin_password', $this->lang->line('admin_password_new'), 'required|valid_password|md5');
	...
	function _password_form($reset, $data)	{
		...
		$this->template->write_view('content', 'user/admin_password_form', $object);
		$this->template->render();
	}
	...
	function profile_edit($admin_id = 0) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$this->l_page->save_page('next', 'referer');
				$data['admin'] = $this->m_user->admin_select($admin_id);
...
//		print_r($admin);die();
		...
		if($this->m_user->admin_update($admin)) {
			...
			$object['refresh_url'] = 'user/browse';
			$object['jag_module'] = $this->jagmodule;
	...
	function perm_group_edit($admin_id) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$this->l_page->save_page('next', 'referer');
				$data['admin'] = $this->m_user->admin_select($admin_id);
//				$data['admin_perm_group_id'] = $this->m_user->admin_perm_groups_select($admin_id);
				$data['admin_perm_group_ids'] = $this->m_perm->admin_perm_group_ids_select($admin_id);
				$this->_perm_group_form(0, $data);
			}
	...
	function _perm_group_form($reset=0, $data=NULL) {
		...
		$object['perm_groups'] = $this->m_perm->perm_groups_select_all();
		$this->template->write_view('content', 'user/admin_perm_group_form', $object);
		$this->template->render();
	}
	...
	function _perm_group_update()	{
		...
		$admin = $this->m_user->admin_select($_POST['admin_id']);

//		$this->m_user->admin_perm_group_delete($group);
...
//		$admin_perm_group_ids = $this->m_perm->admin_perm_group_ids_select($_POST['admin_id']);
		...
		if($this->m_user->admin_update($data)) {
			$object['refresh'] = 1;
