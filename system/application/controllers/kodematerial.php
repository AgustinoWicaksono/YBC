<?php
class Kodematerial extends Controller {
	private $jagmodule = array();


	function Kodematerial() {
		parent::Controller();
		$this->jagmodule = $this->m_config->system_web_module(1036);  //get module data from module ID
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_kodematerial'))
			redirect('');
	...
	function browse() {
		...
		$object['page_title'] = 'List Kode Material di Outlet';
		$this->template->write_view('content', 'kodematerial/kodematerial_browse', $object);
	...
	function browse_search() {
		...
		redirect('kodematerial/browse_result/'.$date_from."/".$date_to);
	...
	function browse_result($date_from = '', $date_to = '')	{
		...
		$this->l_page->save_page('kodematerial_browse_result');
...
//			$object['data']['date_from'] = '';
		...
		for($a = 0; $a < $days_interval; $a++) {
			...
			$filename = './report/'.$sap_client['CLIENT'].'_MatDoc_'.date('Ymd', $timestamp_check).'.csv';
...
//					$num = count($data[$i][$j]);
		...
		$this->load->view('kodematerial/kodematerial_browse_result', $object);
