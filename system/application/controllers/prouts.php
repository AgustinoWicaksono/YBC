<?php
class prouts extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_outs_pr'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Outstanding PR  ';
		$this->template->write_view('content', 'prouts/prouts_browse', $object);
	...
	function browse_search() {
		...
		redirect('prouts/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='',$date_to = '')	{
		...
		$this->l_page->save_page('prouts_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		$this->load->view('prouts/prouts_browse_result', $object);
