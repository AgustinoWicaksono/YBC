<?php
class icecream extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_icecream'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_icecream->icecream_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Ice Cream ';
		$this->template->write_view('content', 'icecream/icecream_browse', $object);
	...
	function browse_search() {
		...
		redirect('icecream/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='', $date_to = '')	{
		...
		$this->l_page->save_page('icecream_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_icecream->icecream_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_icecream');
		//$data['data'] = $this->m_icecream->icecream_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('icecream/icecream_browse_result', $object);
		$this->load->view('icecream/icecream_browse_result', $object);
