	...
	function _input_add() {
		...
		if(isset($_POST['button']['save']) || isset($_POST['button']['approve'])) {
			...
			//echo 'kampret2';
			  ...
			  //echo 'kampret3';
   			    $this->l_general->success_page('Data Transfer Out Inter Outlet berhasil dimasukkan', site_url('gistonew_dept/input'));
              ...
              if($approve_data_success === TRUE) {
  			     $this->l_general->success_page('Data Transfer Out Inter Outlet berhasil diapprove', site_url('gistonew_dept/input'));
              } 
			  else if ($input_batch_success === FALSE)
			  {
			  	$this->m_gistonew_dept->gistonew_dept_header_delete($id_gistonew_dept_header);
...
//       exit;
    ...
    */
            ...
            if(isset($_POST['button']['save'])) {
              if ($input_detail_success == TRUE) {
			  $this->l_general->success_page('Data Transfer Out Inter Outlet berhasil diubah', site_url('gistonew_dept/browse'));
              } else {
		...
		$this->l_general->error_page($this->jagmodule, 'Data Transfer Out Inter Outlet tidak berhasil diubah', site_url($this->session->userdata['PAGE']['next']));
              ...
              if($approve_data_success == TRUE) {
			    $this->l_general->success_page('Data Transfer Out Inter Outlet berhasil diapprove', site_url('gistonew_dept/browse'));
              } else {
...
//			$gistonew_dept_header['id_gistonew_dept_header'] = '0001'.date("Ymd").sprintf("%04d", $running_number);
           ...
           $this->db->trans_complete();
            ...
            if($cancel_data_success == TRUE) {
			    $this->l_general->success_page('Data Transfer Out Inter Outlet berhasil dibatalkan', site_url('gistonew_dept/browse'));
            } else {
	...
	function _delete($id_gistonew_dept_header) {
	  ...
	  $gistonew_dept_delete['module'] = "Transfer Out Outlet";
...
//                            if($gistonew_dept_receiving_temp = $this->m_general->sap_plants_select_all_non_ck_outlet()){
	...
	{
		$this->load->model('m_printgisto_new');
		$data['data'] = $this->m_printgisto_new->tampil($id_gistonew_dept_header);
		
		ob_start();
		$content = $this->load->view('gistonew_dept',$data);
