<?php
class bincard extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_bincard'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_bincard->bincard_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Bin Card Report ';
		$this->template->write_view('content', 'bincard/bincard_browse', $object);
	...
	function browse_search() {
		...
		$date_to =$_POST['to'];
        $group1 = $_POST['WhsCode1'];
        $group2 = $_POST['DocEntry'];
        $group3 = $_POST['DocEntry2'];

		redirect('bincard/browse_result/'.$date_from.'/'.$group1.'/'.$group2.'/'.$group3.'/'.$date_to);

	}

	// search result
	function browse_result($date_from = '', $group1 = '', $group2 = '', $group3 = '', $date_to='')	{

		$this->l_page->save_page('bincard_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_bincard->bincard_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_bincard');
		//$data['data'] = $this->m_bincard->bincard_select_matr($date_from);
		$object['date_from']=$date_from;
		$object['date_to']=$date_to;
		$object['group1']=$group1;
		$object['group2']=$group2;
		$object['group3']=$group3;
		
		//echo $object['date'];
	//	$object['data'] = $data;
		$object['page_title'] = 'Bin Card Report';

		//$this->load->view('bincard/bincard_browse_result', $object);
		$this->load->view('bincard/bincard_browse_result', $object);

	}
	
	function browse_result_excel($date_from, $group1, $group2, $group3, $date_to)	{

		$this->l_page->save_page('bincard_browse_result_excel');
	/*	if($date_from !== '0') {
			$year = substr($date_from, 0, 4);
			$month = substr($date_from, 4, 2);
			$day = substr($date_from, 6, 2);
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('bincard/bincard_browse_result', $object);
		$this->load->view('bincard/bincard_browse_result_excel', $object);
