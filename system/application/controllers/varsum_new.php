<?php
class varsum_new extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_variance_summary'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$this->template->write_view('content', 'varsum_new/varsum_new_browse', $object);
	...
	function browse_search() {
		...
		redirect('varsum_new/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='',$date_to = '')	{
		...
		$this->l_page->save_page('varsum_new_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		$this->load->view('varsum_new/varsum_new_browse_result', $object);
