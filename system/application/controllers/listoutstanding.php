<?php
class Listoutstanding extends Controller {
	private $jagmodule = array();


	function Listoutstanding() {
		parent::Controller();
		$this->jagmodule = $this->m_config->system_web_module(1037);  //get module data from module ID
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_listoutstanding'))
			redirect('');
	...
	function browse() {
		...
		$object['page_title'] = 'List Outstanding PR/PO/Delivery vs Stock vs Requirements';
		$this->template->write_view('content', 'listoutstanding/listoutstanding_browse', $object);
	...
	function browse_search() {
		...
		redirect('listoutstanding/browse_result/'.$date_from."/".$date_to);
	...
	function browse_result($date_from = '', $date_to = '')	{
		...
		$this->l_page->save_page('listoutstanding_browse_result');
...
//			$object['data']['date_from'] = '';
		...
		for($a = 0; $a < $days_interval; $a++) {
			...
			$filename = './report/'.$sap_client['CLIENT'].'_ListPRStatus_'.date('Ymd', $timestamp_check).'.csv';
...
//					$num = count($data[$i][$j]);
		...
		$this->load->view('listoutstanding/listoutstanding_browse_result', $object);
