<?php
class onhand extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_onhand'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_onhand->onhand_get_period_selection() as $data) {
		...
		$this->template->write_view('content', 'onhand/onhand_browse', $object);
	...
	function browse_search() {
		...
		redirect('onhand/browse_result/'.$group);
	...
	function browse_search_print() {
		...
		redirect('onhand/browse_result_excel/'.$group1);

	}

	// search result
	function browse_result($group = '')	{

		$this->l_page->save_page('onhand_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_onhand->onhand_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_onhand');
		//$data['data'] = $this->m_onhand->onhand_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('onhand/onhand_browse_result', $object);
		$this->load->view('onhand/onhand_browse_result', $object);

	}
	
	function browse_result_excel($group)	{

		$this->l_page->save_page('onhand_browse_result_excel');
	/*	if($date_from !== '0') {
			$year = substr($date_from, 0, 4);
			$month = substr($date_from, 4, 2);
			$day = substr($date_from, 6, 2);
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('onhand/onhand_browse_result', $object);
		$this->load->view('onhand/onhand_browse_result_excel', $object);
