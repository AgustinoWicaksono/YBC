<?php
class returout extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_returout'))
			redirect('');
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Retur Out  ';
		$this->template->write_view('content', 'returout/returout_browse', $object);
	...
	function browse_search() {
		...
		redirect('returout/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='',$date_to = '')	{
		...
		$this->l_page->save_page('returout_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
	...
	//	$object['data'] = $data;
		...
		$this->load->view('returout/returout_browse_result', $object);
