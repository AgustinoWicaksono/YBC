<?php
class cakeshop extends Controller {
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_cakeshop'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_cakeshop->cakeshop_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Cake Shop ';
		$this->template->write_view('content', 'cakeshop/cakeshop_browse', $object);
	...
	function browse_search() {
		...
		redirect('cakeshop/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '', $group = '',$date_to='')	{
		...
		$this->l_page->save_page('cakeshop_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_cakeshop->cakeshop_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_cakeshop');
		//$data['data'] = $this->m_cakeshop->cakeshop_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('cakeshop/cakeshop_browse_result', $object);
		$this->load->view('cakeshop/cakeshop_browse_result', $object);
