...
class Admin extends Controller {
	...
	function Admin() {
		...
		parent::Controller();
		$this->jagmodule = $this->m_config->system_web_module(1062);  //get module data from module ID
...
;
	...
	function browse() {
		...
		if(!empty($admin_browse_result))
			...
			redirect('admin/browse_result/0/0/0/0/10');
...
//		redirect('admin/browse_result/'.$_POST['field_name']."/".$_POST['field_type']."/".$field_content."/".$date_from."/".$date_to."/".$_POST['status']."/".$_POST['sort1']."/".$_POST['sort2']."/".$_POST['sort3']."/".$limit);
		redirect('admin/browse_result/'.$_POST['field_name']."/".$_POST['field_type']."/".$field_content."/0/".$limit);
...
*/
		...
		$sort_link1 = 'admin/browse_result/'.$field_name.'/'.$field_type.'/'.$field_content.'/';
		$sort_link2 = '/'.$limit.'/';
		...
		$config['base_url'] = site_url('admin/browse_result/'.$field_name.'/'.$field_type.'/'.$field_content.'/'.$sort.'/'.$limit.'/');

		$config['total_rows'] = $this->m_admin->admin_count_by_criteria($field_name, $field_type, $field_content, $sort);;

		// save pagination definition
		$this->pagination->initialize($config);

		$object['data']['admins'] = $this->m_admin->admin_select_by_criteria($field_name, $field_type, $field_content, $sort, $limit, $start);

		$object['limit'] = $limit;
		$object['total_rows'] = $config['total_rows'];

		$object['page_title'] = 'Managemen Pengguna';
		$this->template->write_view('content', 'admin/admin_browse', $object);
	...
	function view($admin_id = 0) {
		...
		$object['data'] = $this->m_admin->admin_select($admin_id);

		// save this page to referer
		$this->l_page->save_page('next');

		$this->template->write_view('content', 'admin/admin_view', $object);
		$this->template->render();
	...
	function delete($admin_id) {
		...
		$this->m_admin->admin_delete($admin_id);
...
//			$member['data']['membership_status'] = 2;
		...
		if(!empty($data['plant_type'])) {
			redirect('admin/input2/'.$data['plant_type']);
		}

		$object['page_title'] = 'Manajemen Administrator';
		$this->template->write_view('content', 'admin/admin_input_form', $object);
		$this->template->render();
	...
	function _input_check() {
		$this->form_validation->set_rules('admin[admin_username]', $this->lang->line('admin_username'), 'trim|required|username_exist');
		$this->form_validation->set_rules('admin[admin_realname]', $this->lang->line('admin_realname'), 'trim|required|');
...
//			$admin['data']['membership_status'] = 2;
		...
		$this->template->write_view('content', 'admin/admin_profile_form', $object);
		$this->template->render();
	}
    ...
    $perm_group_id = $_POST['perm_group_id'];
		...
		$admin_id = $this->m_admin->admin_add($admin);
	...
	function _input_success() {
		...
		$object['refresh_url'] = site_url('admin/input');
	...
	function password_edit($admin_id = 0) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$data = $this->m_admin->admin_select($admin_id);
				$this->_password_form(0, $data);
			}
...
//		$this->form_validation->set_rules('admin_password', $this->lang->line('admin_password_new'), 'required|valid_password|md5');
	...
	function _password_form($reset, $data)	{
		...
		$this->template->write_view('content', 'admin/admin_password_form', $object);
		$this->template->render();
	}
	...
	function profile_edit($admin_id = 0) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$this->l_page->save_page('next', 'referer');
				$data['admin'] = $this->m_admin->admin_select($admin_id);
	...
	function _profile_form($reset=0, $data=NULL) {
		...
		$object['plants'] = $this->m_general->sap_plants_select_all($plant_type_id);
		//$object['selectall'] = $this->m_general->sap_plants_selectall_type($plant_type_id);
	...
	function _profile_update()	{
		...
		if($this->m_admin->admin_update($admin)) {
			...
			$object['error_text'] = sprintf($this->lang->line('error_admin_profile_update'), $admin['admin_name']);
	...
	function perm_group_edit($admin_id) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_POST)) {
				...
				$this->l_page->save_page('next', 'referer');
				$data['admin'] = $this->m_admin->admin_select($admin_id);
//				$data['admin_perm_group_id'] = $this->m_admin->admin_perm_groups_select($admin_id);
				$data['admin_perm_group_ids'] = $this->m_perm->admin_perm_group_ids_select($admin_id);
				$this->_perm_group_form(0, $data);
			}
	...
	function _perm_group_form($reset=0, $data=NULL) {
		...
		$object['perm_groups'] = $this->m_perm->perm_groups_select_all();
		$this->template->write_view('content', 'admin/admin_perm_group_form', $object);
		$this->template->render();
	}
	...
	function _perm_group_update()	{
		...
		$admin = $this->m_admin->admin_select($_POST['admin_id']);

//		$this->m_admin->admin_perm_group_delete($group);
...
//		$admin_perm_group_ids = $this->m_perm->admin_perm_group_ids_select($_POST['admin_id']);
		...
		if($this->m_admin->admin_update($data)) {
			$object['refresh'] = 1;
			...
			$object['refresh'] = 1;
			$object['error_text'] = sprintf($this->lang->line('error_admin_perm_group_update'), $admin['admin_name']);
			$object['jag_module'] = $this->jagmodule;
