...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_bar'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_bar->bar_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Bar ';
		$this->template->write_view('content', 'bar/bar_browse', $object);
	...
	function browse_search() {
		...
		redirect('bar/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '', $group = '',$date_to='')	{
		...
		$this->l_page->save_page('bar_browse_result');
	/*	if($date_from !== '0') {
