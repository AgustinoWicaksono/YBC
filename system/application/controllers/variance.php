<?php
class Variance extends Controller {
	private $jagmodule = array();


	function Variance() {
		parent::Controller();
		$this->jagmodule = $this->m_config->system_web_module(1058);  //get module data from module ID
...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_variance'))
			redirect('');
	...
	function browse() {
		...
		$this->template->write_view('content', 'variance/variance_browse', $object);
	...
	function browse_search() {
		...
		redirect('variance/browse_result/'.$date_from."/".$date_to);
	...
	function browse_result($date_from = '', $date_to = '')	{
		...
		$this->l_page->save_page('variance_browse_result');
...
//					$num = count($data[$i][$j]);
            		...
            		if($data[$i][$j][0] == $this->session->userdata['ADMIN']['plant']) { // check is it the plant's data?
    					...
    					$subtotal[$i][16] += $data[$i][$j][16];
                         /*
    					$total[5] += $subtotal[$i][5];
    					$total[9] += $subtotal[$i][9];
    					$total[12] += $subtotal[$i][12];
    					$total[14] += $subtotal[$i][14];
    					$total[16] += $subtotal[$i][16];
                        */
    					$total[5] += $data[$i][$j][5];
    					$total[9] += $data[$i][$j][9];
		...
		$this->load->view('variance/variance_browse_result', $object);
