...
//			$this->l_auth->save_referer_and_login();
	...
	function browse() {
     ...
     /*   foreach($this->m_bread->bread_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Bread ';
		$this->template->write_view('content', 'bread/bread_browse', $object);
	...
	function browse_search() {
		...
		redirect('bread/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '',$group='',$date_to = '')	{
		...
		$this->l_page->save_page('bread_browse_result');
	/*	if($date_from !== '0') {
