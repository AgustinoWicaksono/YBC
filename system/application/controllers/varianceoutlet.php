<?php
class varianceoutlet extends Controller {
	private $jagmodule = array();


	function varianceoutlet() {
		parent::Controller();
...
//			$this->l_auth->save_referer_and_login();
		...
		$this->load->model('m_varianceoutlet');
	...
	function browse() {
        ...
        foreach($this->m_varianceoutlet->varianceoutlet_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Report Variance Outlet';
		$this->template->write_view('content', 'varianceoutlet/varianceoutlet_browse', $object);
	...
	function browse_search() {
        $date_from = $this->l_general->leftstr($_POST['periode'],8);
        $date_to = $this->l_general->rightstr($_POST['periode'],8);

		redirect('varianceoutlet/browse_result/'.$date_from."/".$date_to);
	...
	function browse_result($date_from = '', $date_to = '')	{
		...
		$this->l_page->save_page('varianceoutlet_browse_result');
...
//			$object['data']['date_from'] = '';
    	...
    	foreach ($this->m_varianceoutlet->varianceoutlet_select_matr($date_from,$date_to) as $object['temp']) {
    	    $j = 0;
		...
		$this->load->view('varianceoutlet/varianceoutlet_browse_result', $object);
