...
//			$this->l_auth->save_referer_and_login();
		...
		if(!$this->l_auth->is_have_perm('report_userdelete'))
			redirect('');
	...
	function browse() {
	  ...
	  /*   foreach($this->m_userdelete->userdelete_get_period_selection() as $data) {
		...
		$object['page_title'] = 'Delete History ';
		$this->template->write_view('content', 'userdelete/userdelete_browse', $object);
	...
	function browse_search() {
		...
		redirect('userdelete/browse_result/'.$date_from.'/'.$group.'/'.$date_to);
	...
	function browse_result($date_from = '', $group = '',$date_to='')	{
		...
		$this->l_page->save_page('userdelete_browse_result');
	/*	if($date_from !== '0') {
...
//			$object['data']['date_from'] = '';
    	...
    	/*foreach ($this->m_userdelete->userdelete_select_matr($date_from) as $object['temp']) {
    	    $j = 0;
		...
		//$this->load->model('m_userdelete');
		//$data['data'] = $this->m_userdelete->userdelete_select_matr($date_from);
	...
	//	$object['data'] = $data;
		...
		//$this->load->view('userdelete/userdelete_browse_result', $object);
		$this->load->view('userdelete/userdelete_browse_result', $object);
	...
	public function cetak($id_stdstock_header)
	{
		...
		$this->load->view('userdelete_room',$data);
	}	
