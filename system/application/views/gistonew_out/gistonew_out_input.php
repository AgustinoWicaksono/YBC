...
<table width="1038" border="0" align="center">
  <tr>
    ...
    <td class="table_field_h"><?php echo $this->m_gistonew_out->posto_lastupdate(); ?>
  ...
  <tr>
    ...
    <td class="column_input"><?=form_hidden('do_no', $data['do_no']);?>
        <?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="chosen-select input_text" disabled="disabled"');?>
        <?php if(!empty($_POST)) : if(!empty($data['do_no'])) : echo anchor('gistonew_out/input', '<strong>Pilih ulang SR No</strong>'); endif; endif;?>
    </td>
    <?php else : ?>
    ...
    <td class="column_input">Anda memiliki <b style="color:#aa1122;"><?php echo count($do_no)-1; ?> nomor SR</b> yang harus <b><u>segera</u></b> diterima dan disetujui.
        <hr />
      <?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="chosen-select input_text" onChange="document.form1.submit();"');?></td>
    <?php } else { ?>
  ...
  <tr>
    ...
    <td class="column_input"><?php if(!empty($data['gistonew_out_header']['DELIV_DATE'])) : ?>
        <?=$this->l_general->sqldate_to_date($data['gistonew_out_header']['DELIV_DATE']);?>
      <?php endif; ?></td>
  </tr>
  <tr>
