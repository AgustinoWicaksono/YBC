...
<script language="javascript">
	function confirm_gistonew_out_delete(url, id_gistonew_out_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Issue Antar Department  dengan ID "' + id_gistonew_out_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<table width="1340"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
...
<?php endif; ?>
<?php
$outlet=$gistonew_out_header['receiving_plant'];
...
?>
	...
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('gistonew_out/excel/'.$gistonew_out_header['id_gistonew_out_header'], 'Export to Excel', 'page_white_excel.png', 'height="20" width="20"');?> <?=anchor_image('gistonew_out/printpdf/'.$gistonew_out_header['id_gistonew_out_header'], 'Print Transaksi', 'print.png', 'height="20" width="20"');?><?=anchor_image('gistonew_out/edit/'.$gistonew_out_header['id_gistonew_out_header'].'/'.$gistonew_out_header['receiving_plant'].'/'.$gistonew_out_header['item_group_code'].'/1', 'Ubah dengan Item berdasarkan ID', 'edit.png', 'height="20" width="20"');?> <?=anchor_image('gistonew_out/edit/'.$gistonew_out_header['id_gistonew_out_header'].'/'.$gistonew_out_header['receiving_plant'].'/'.$gistonew_out_header['item_group_code'].'/2', 'Ubah dengan Item berdasarkan Nama', 'view.png', 'height="20" width="20"');?> <?php if($gistonew_out_header['status'] == 1) : ?><a href="#" onClick='confirm_gistonew_out_delete("<?=site_url('gistonew_out/delete/'.$gistonew_out_header['id_gistonew_out_header']);?>", "<?=$gistonew_out_header['id_gistonew_out_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<!--td class="table_content_1" nowrap="nowrap"><?//=$gistonew_out_header['plant'].'04';?><?=date("ymd", strtotime($gistonew_out_header['posting_date']));?><?=sprintf("%06d", $gistonew_out_header['id_gistonew_out_plant']);?></td-->
    <td class =""><?=$gistonew_out_header['id_gistonew_out_header'];?></td>
    <td class =""><?=$gistonew_out_header['gistonew_out_no'];?></td>
	...
	<td><?=$gistonew_out_header['status_string'];?></td>
		<?php
			$id=$gistonew_out_header['id_user_input'];
			$r=mysql_fetch_array(mysql_query("SELECT admin_realname FROM d_admin WHERE admin_id = '$id'"));
			$name=$r['admin_realname'];
		?>
		<td ><?php echo $name;?></td>
		<td ><?php if ($gistonew_out_header['status']== 2){
		  $id=$gistonew_out_header['id_user_approved'];
		  $r=mysql_fetch_array(mysql_query("SELECT admin_realname FROM d_admin WHERE admin_id = '$id'"));
		  $name=$r['admin_realname'];
		  echo $name; }
          else {echo "-";}?></td>
	
	<td><?=$gistonew_out_header['lastmodified'];?></td>
	 <td align="center"><?php $log=$gistonew_out_header['back'];
		$po=$gistonew_out_header['gistonew_out_no'];
		if ($log==0 && $po !='' && $po !='C')
		{
			echo "Integrated";
...
<?=form_close();?>
<?=form_open_multipart('gistonew_out/file_import');?>
