...
<table width="1038" border="0" align="center">
	...
	<?php if(!empty($data['item_choose'])) : ?>
        ...
        <?php if((!empty($_POST))&&(!empty($data['item']))) : ?>
           ...
           <?=form_hidden('mpaket_prod_header[kode_paket]', $data['item']);?><?=$data['item'];?></td>
           <tr>
  		   <td class="table_field_1"><strong>Nama Item BOM </strong></td>
           <td class="column_input"><?=form_hidden('mpaket_prod_header[nama_paket]', $item_paket_name);?><?=$item_paket_name;?></td>
           
           </tr>
        <?else : ?>
          <td class="column_input"><?=form_dropdown('item', $item, $data['item'], 'class="input_text"  onChange="document.form1.submit();"');?>
