...
<?php endif; ?>
    ...
    <?php
			...
			$res=mysql_fetch_array(mysql_query("SELECT OUTLET_NAME1 FROM m_outlet WHERE TRANSIT = '$gistonew_dept_header[receiving_plant]' "));
			$name_outlet=$res['OUTLET_NAME1'];
		?>

	<td class = ""><?=date("d-m-Y", strtotime($gistonew_dept_header['posting_date']));?></td>
	<td class = ""><?=$name_outlet;?></td>
