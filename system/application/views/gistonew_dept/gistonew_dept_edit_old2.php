...
?>
  ...
  <tr>
    ...
    <td class="column_input"><strong>
      <?=form_hidden('gistonew_dept_header[do_no]', $gistonew_dept_header['VBELN']);?>
      <?=$gistonew_dept_header['VBELN']; ?>
      </strong>
        <?php if($function == 'input2') echo ' '.anchor('gistonew_dept/input', '<strong>Pilih ulang SR Number dan Jenis Material</strong>');?></td>
  </tr>
  <tr>
...
<?php endif; ?>
        ...
        <?=form_hidden("gistonew_dept_detail[posnr][$k]", $item_x[$i]['POSNR']);?>
        <?php endif; ?><?=$k;?></td>
...
OWTQ.DocEntry = '$gistonew_dept_header[VBELN]'
		...
		if(!empty($error) && in_array("gistonew_dept_detail[num][$i]", $error)) {
			echo form_dropdown("gistonew_dept_detail[num][$k]", $item2, $data['gistonew_dept_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select error_text" style="width:450px;" ');
		} else {
		echo form_dropdown("gistonew_dept_detail[num][$k]", $item2  , $data['gistonew_dept_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select input_text" style="width:450px;"  ');  
		}
		}
		else
		{
		...
		{
		//echo $qty[$i];
		echo number_format($qty[$i], 3, '.', ',');?><?=form_hidden("gistonew_dept_detail[outstanding_qty][$i]",$qty[$i]);
		}
		?>
		</td>
		<td align="center">
		...
		{
		if(!empty($error) && in_array("gistonew_dept_detail[gr_quantity][$i]", $error)) {
			echo form_input("gistonew_dept_detail[gr_quantity][$k]",$qty[$i], $data['gistonew_dept_detail']['gr_quantity'][$i], 'class="error_number" size="8" ');
		} else {
			echo form_input("gistonew_dept_detail[gr_quantity][$k]",$qty[$i], $data['gistonew_dept_detail']['gr_quantity'][$i], 'class="input_number" size="8" ');
		}
		}
		else
		{
