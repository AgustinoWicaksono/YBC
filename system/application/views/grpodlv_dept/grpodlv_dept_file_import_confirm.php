...
?>
<?=form_open('grpodlv_dept/file_import_execute', $form1);?>
...
if($grpodlv_dept_headers !== FALSE) :
	$i = 1;
	foreach ($grpodlv_dept_headers as $grpodlv_dept_header) :
?>
	<tr>
        <td class="table_content_1"><?=$grpodlv_dept_header['grpodlv_dept_no'];?></td>
		<td class="table_content_1"><?=$grpodlv_dept_header['do_no'];?></td>     
			<td class="table_content_1"><?=$grpodlv_dept_header['plant'];?></td>
		<td class="table_content_1"><?=$grpodlv_dept_header['storage_location'];?></td>
...
<?php
		$j = 1;
		foreach ($grpodlv_dept_details[$i] as $grpodlv_dept_detail[$i]) :
?>
	<tr>
		<td class="table_row_2"><?=$j;?></td>
		<td class="table_row_2"><?=$grpodlv_dept_detail[$i]['material_no'];?></td>
		<td class="table_row_2"><?=$grpodlv_dept_detail[$i]['material_desc'];?></td>
		<td class="table_row_2"><?=$grpodlv_dept_detail[$i]['outstanding_qty'];?></td>
		<td class="table_row_2"><?=$grpodlv_dept_detail[$i]['gr_quantity'];?></td>
		<td class="table_row_2"><?=$grpodlv_dept_detail[$i]['uom'];?></td>
	</tr>
<?php
			$j++;
		endforeach;
