...
<?php /* <div style="color:#ffffff;font-style:italic;font-size:90%;">Jika data tidak lengkap, harap tunggu sinkronisasi berikutnya, yaitu 2 jam dari waktu di atas.</div> */ ?>    </td>
	...
	if(!empty($data['do_no'])) : ?>
	    ...
	    <?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="input_text" disabled="disabled"');?>		  <?php if(!empty($_POST)) : if(!empty($data['do_no'])) : echo anchor('grpodlv_dept/input', '<strong>Pilih ulang Transfer Slip Number</strong>'); endif; endif;?>        </td>
<?php else : ?>
		<?php if (count($do_no)>0) { ?>
		<td class="column_input">Anda memiliki <b style="color:#aa1122;"><?php echo count($do_no)-1; ?><strong> Transfer Slip Number</strong></b> yang harus <b><u>segera</u></b> diterima dan disetujui.
	      <hr /><?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="input_text" onChange="document.form1.submit();"');?></td>
    <?php } else { ?>
		<td class="column_input"><b style="color:#aa1122;">Saat ini tidak ada <strong>Transfer Slip Number</strong> </b><br />
...
<?php if(isset($_POST['do_no'])) : ?>
	...
	<tr> 
		...
		<td class="column_input"><?php if(!empty($data['grpodlv_dept_header']['DELIV_DATE'])) : ?><?=$this->l_general->sqldate_to_date($data['grpodlv_dept_header']['DELIV_DATE']);?><?php endif; ?></td>
	</tr>
	<tr>
