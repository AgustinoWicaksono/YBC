...
<script language="javascript">
	function confirm_grpodlv_dept_delete(url, id_grpodlv_dept_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO dengan ID "' + id_grpodlv_dept_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<p>&nbsp;</p>
<?=form_open('grpodlv_dept/browse_search', $form1);?>
...
<?=form_open('grpodlv_dept/delete_multiple_confirm', $form2);?>
<table width="1000"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="159" class = "table_header_1" align="center" ><strong>Transfer In  No<br /><?=anchor_image($sort_link['cy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['cz'],'Z-A', 'panahatas.jpg');?></strong></td>
...
if($data['grpodlv_dept_headers'] !== FALSE) :
	$i = 1;
	foreach ($data['grpodlv_dept_headers']->result_array() as $grpodlv_dept_header):
		$grpodlv_dept_header['status_string'] = ($grpodlv_dept_header['status'] == '2') ? 'Approved' : 'Not Approved';
        if($this->m_grpodlv_dept->grpodlv_dept_is_data_cancelled($grpodlv_dept_header['id_grpodlv_dept_header']))
           $grpodlv_dept_header['cancel_status'] = 'Y';
        else
           $grpodlv_dept_header['cancel_status'] = 'N';
?>
...
<?php endif; ?>
   <td class=""><?=form_checkbox('id_grpodlv_dept_header['.$i.']', $grpodlv_dept_header['id_grpodlv_dept_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('grpodlv_dept/printpdf/'.$grpodlv_dept_header['id_grpodlv_dept_header'], 'Print Transaksi', 'print.png', 'height="20" width="20"');?><?=anchor_image('grpodlv_dept/edit/'.$grpodlv_dept_header['id_grpodlv_dept_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <?php if($grpodlv_dept_header['status'] == 1) : ?><a href="#" onClick='confirm_grpodlv_dept_delete("<?=site_url('grpodlv_dept/delete/'.$grpodlv_dept_header['id_grpodlv_dept_header']);?>", "<?=$grpodlv_dept_header['id_grpodlv_dept_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="table_content_1" nowrap="nowrap"><?=$grpodlv_dept_header['plant'].'02';?><?=date("ymd", strtotime($grpodlv_dept_header['posting_date']));?><?=sprintf("%06d", $grpodlv_dept_header['id_grpodlv_dept_plant']);?></td>
    <td class =""><?=$grpodlv_dept_header['do_no'];?></td>
    <td class = ""><?=$grpodlv_dept_header['grpodlv_dept_no'];?></td>
	<!--td><?php if(!empty($grpodlv_dept_header['delivery_date'])) : ?><?=$this->l_general->sqldate_to_date($grpodlv_dept_header['delivery_date']);?><?php endif; ?></td-->
	<td class = ""><?=date("d-m-Y", strtotime($grpodlv_dept_header['posting_date']));?></td>
	<td><?=$grpodlv_dept_header['status_string'];?></td>
	<td align="center"><?=$grpodlv_dept_header['cancel_status'];?></td>
...
<?=form_close();?>
<?=form_open_multipart('grpodlv_dept/file_import');?>
<table width="1000" align="center" class ="table_border_1">
  <tr>
    <td width="220" >&nbsp;</td>
    <td width="330" >&nbsp;</td>
    <td width="354" colspan="3" class="space"><?=anchor('grpodlv_dept/input', 'Add New Goods Receipt');?></td>
  </tr>
</table>
<?=form_close();?>
<table class="table-browse" width="100%" align="center">
