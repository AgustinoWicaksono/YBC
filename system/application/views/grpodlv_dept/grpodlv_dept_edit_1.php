...
?>
  <tr>
    <td width="272" class="table_field_1"><strong>Delivery Order Number </strong></td>
		<td class="column_input"><?=form_hidden('grpodlv_dept_header[do_no]', $grpodlv_dept_header['VBELN']);?><?=$grpodlv_dept_header['VBELN'];?></td>
  </tr>
	<tr>
		<td class="table_field_1"><strong>Goods Receipt Number </strong></td>	
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['grpodlv_dept_header']['grpodlv_dept_no'];?></td>
	</tr>
	<tr>
...
?>
		<td class="column_input"><?php if(($data['grpodlv_dept_header']['status'] !== '2')&&($this->l_auth->is_have_perm('auth_backdate'))) : ?>
		...
		</script><?php else: ?><?=$posting_date;?><?=form_hidden("grpodlv_dept_header[posting_date]", $posting_date);?><?php endif;?></td>
	</tr>
</table>
...
?>
  ...
  <tr class="table_row_1">
    ...
    <td><?php if(!isset($data['grpodlv_dept_header']['status']) || $data['grpodlv_dept_header']['status'] == '1') : ?>
      ...
      <?=form_hidden("grpodlv_dept_detail[item][$i]", $grpodlv_dept_detail['POSNR'][$i]);?>
      <?php endif; ?>
      <?=$i;?></td>
		<td><?php if($function=='edit'):?><?=$data['grpodlv_dept_detail']['material_no'][$i];?><?php else: ?><?=$grpodlv_dept_detail['MATNR'][$i];?><?php endif;?></td>
		<td><?php if($function=='edit'):?><?=$data['grpodlv_dept_detail']['material_desc'][$i];?><?php else: ?><?=$grpodlv_dept_detail['MAKTX'][$i];?><?php endif;?></td>
		<td align="center"><?php if($function=='edit'):?><?=$data['grpodlv_dept_detail']['outstanding_qty'][$i];?><?php else: ?><?=$grpodlv_dept_detail['LFIMG'][$i];?><?php endif;?></td>
        <td align="center"><?php
		if($data['grpodlv_dept_header']['status'] != '2') {
			if(!empty($error) && in_array("grpodlv_dept_detail[gr_quantity][$i]", $error)) {
				echo form_input("grpodlv_dept_detail[gr_quantity][$i]", $data['grpodlv_dept_detail']['gr_quantity'][$i], 'class="error_number" size="6"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grpodlv_dept_detail[gr_quantity][$i]", $gr_quantity[$i], 'class="input_number" size="6"');
			} else {
          	echo form_input("grpodlv_dept_detail[gr_quantity][$i]", $data['grpodlv_dept_detail']['gr_quantity'][$i], 'class="input_number" size="6"');
            }
			}
		} else {
			echo $data['grpodlv_dept_detail']['gr_quantity'][$i];
		}
		?></td>
		<td><?php
        if($function=='edit')
    ...
    <td align="center"><?php if($data['grpodlv_dept_detail']['ok_cancel'][$i]==FALSE): ?>
        <?=form_checkbox('cancel['.$i.']', $grpodlv_dept_detail['id_grpodlv_dept_h_detail'][$i], FALSE);?>
      <?php else: ?>
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td align="center"><?php if($data['grpodlv_dept_header']['status'] == '2') : ?>
        ...
        <?php endif; ?>
        <?php endif; ?>
        <?=anchor('grpodlv_dept/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
