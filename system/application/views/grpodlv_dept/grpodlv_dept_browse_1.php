...
<script language="javascript">
	function confirm_grpodlv_dept_delete(url, id_grpodlv_dept_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO from Vendor dengan ID "' + id_grpodlv_dept_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<?php endif; ?>
	...
	<td class="table_content_1" nowrap="nowrap"><?=$grpodlv_dept_header['plant'];?><?=date("ymd", strtotime($grpodlv_dept_header['posting_date']));?><?=sprintf("%06d", $grpodlv_dept_header['id_grpodlv_dept_plant']);?></td>
