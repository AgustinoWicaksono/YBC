...
<?php /* <div style="color:#ffffff;font-style:italic;font-size:90%;">Jika data tidak lengkap, harap tunggu sinkronisasi berikutnya, yaitu 2 jam dari waktu di atas.</div> */ ?>    </td>
  ...
  <td class="column_input"><strong><?=form_hidden('grpodlv_dept_header[do_no]', $grpodlv_dept_header['VBELN']);?><?=$grpodlv_dept_header['VBELN'];?></strong>
	...
	<tr>
		<td class="table_field_1"><strong>From Departement </strong> </td>
	...
	<!--tr>
	  ...
	  <td class="column_input"><strong><?=$grpodlv_dept_header['SUPPL_PLANT'];?> <?=" - ";?><?=$grpodlv_dept_header['SPLANT_NAME'];?> </strong>
        <?=form_hidden("grpodlv_dept_header[delivery_plant]", $grpodlv_dept_header['SUPPL_PLANT']);?>
        <?=form_hidden("grpodlv_dept_header[delivery_plant_name]", $grpodlv_dept_header['SPLANT_NAME']);?></td>
  </tr-->
	<tr>
...
<table width="<?php echo $lebar; ?>" border="0" bordercolor="#000000" align="center">
  ...
  <tr>
    <?php if(($data['grpodlv_dept_header']['status'] == '2')&&($this->l_auth->is_have_perm('auth_cancel'))) { ?>
    <td colspan="10" style="padding:0px;text-align:left;margin:0px;" align="left"><?php } else { ?>
    </td>
    <td colspan="9" style="padding:0px;text-align:left;margin:0px;" align="left"><?php } ?>
        <table class="filterable" border="0" width="100%" cellpadding="0" cellspacing="0" />
    </td>
  </tr>
  <tr>
    <td><div style="height:350px;vertical-align:top;overflow:auto;background:none;margin-left:-2px;border-bottom:#376091 1px solid;">
...
//        }
        ...
        ?>
          ...
          <td><?=$material_desc;?>
                <?=form_hidden("grpodlv_dept_detail[material_desc][$i]", $material_desc);?></td>
          <td><?=$grpodlv_dept_detail['material_docno_cancellation'][$i];?>
                <?=form_hidden("grpodlv_dept_detail[material_docno_cancellation][$i]", $grpodlv_dept_detail['material_docno_cancellation'][$i]);?></td>
          <?php
		if ($function=='edit' && $data['grpodlv_dept_header']['status'] != '2')
		{?>
          <td align="right"><?=number_format($outstanding_qty, 2, '.', ',');?>
                <?=form_hidden("grpodlv_dept_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
          <?php }
		else
		{ ?>
          <td align="right"><?=number_format($outstanding_qty, 2, '.', ',');?>
		...
		if($data['grpodlv_dept_header']['status'] != '2') {
			if(!empty($error) && in_array("grpodlv_dept_detail[gr_quantity][$i]", $error)) {
				echo form_input("grpodlv_dept_detail[gr_quantity][$i]", number_format($data['grpodlv_dept_detail']['gr_quantity'][$i], 2, '.', ''), 'class="error_number" size="8"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grpodlv_dept_detail[gr_quantity][$i]", number_format($gr_quantity[$i], 2, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grpodlv_dept_detail[gr_quantity][$i]", number_format($data['grpodlv_dept_detail']['gr_quantity'][$i], 2, '.', ''), 'class="input_number" size="8"');
            }
			}
		} else {
			echo number_format($data['grpodlv_dept_detail']['gr_quantity'][$i], 2, '.', ',');
		}
		?></td>
          <td><?=$uom;?></td>
           <td><?php
        ...
        if(!empty($error) && in_array("grpodlv_dept_detail[val][$i]", $error)) {
			echo form_dropdown("grpodlv_dept_detail[val][$i]", $val, $data['grpodlv_dept_detail']['val'][$i], ' data-placeholder="Pilih material..." class="chosen-select error_text" style="width:50px;" ');
		} else {
			echo form_dropdown("grpodlv_dept_detail[val][$i]", $val, $data['grpodlv_dept_detail']['val'][$i], ' data-placeholder="Pilih material..." class="chosen-select input_text" style="width:50px;" ');
		}
		?></td>
        <td><?php
		if($data['grpodlv_dept_header']['status'] != '2') {
			if(!empty($error) && in_array("grpodlv_dept_detail[var][$i]", $error)) {
				echo form_input("grpodlv_dept_detail[var][$i]", number_format($data['grpodlv_dept_detail']['var'][$i], 2, '.', ''), 'class="error_number" size="4"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grpodlv_dept_detail[var][$i]", number_format($var[$i], 2, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grpodlv_dept_detail[var][$i]", number_format($data['grpodlv_dept_detail']['var'][$i], 2, '.', ''), 'class="input_number" size="4"');
            }
			}
		} else {
			echo number_format($data['grpodlv_dept_detail']['gr_quantity'][$i], 2, '.', ',');
