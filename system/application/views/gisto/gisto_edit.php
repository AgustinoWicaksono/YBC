...
?>
	<tr>
		<td width="272" class="table_field_1"><strong>SR No </strong></td>
		<td class="column_input"><?=empty($data['gisto_header']['po_no']) ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gisto_header']['po_no'];?></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Transfer Slip No </strong></td>
		<td class="column_input"><?=empty($data['gisto_header']['gisto_no']) ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gisto_header']['gisto_no'];?></td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <?=form_hidden("gisto_header[receiving_plant]", $receiving_plant['plant']);?>
        <?=form_hidden("gisto_header[receiving_plant_name]", $receiving_plant['plant_name']);?>
        </td>
	</tr>
	<tr>
...
//	$i = 1; => already defined on FOR looping below
	...
	// for POST data
	if((!isset($_POST['delete']))&&(empty($gisto_detail_tmp))&&(empty($error)))
		$count2 = $count;
	else
		$count2 = $count - 1;
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if(empty($_POST['delete'][$i])) {
            ...
            if (!empty($data['gisto_detail']['material_desc'][$i])) {
              $item_x[$i]['MAKTX'] = $data['gisto_detail']['material_desc'][$i];
            }
			// dari sini
            if (!empty($data['gisto_detail']['uom'][$i])) {
			  $item_x[$i]['uom'] = $data['gisto_detail']['uom'][$i];
            }
			//====================
			 if (!empty($data['gisto_detail']['num'][$i])) {
              $item_x[$i]['num'] = $data['gisto_detail']['num'][$i];
            }
...
<?php endif; ?>
        ...
        <?php endif; ?><?=$k;?></td>
		<td align="left"><?=form_hidden("gisto_detail[material_no][$k]", $data['gisto_detail']['material_no'][$i]);?>
        <?=$item_x[$i]['MATNR1'];?></td>
		<td align="left"><?=$item_x[$i]['MAKTX'];?>
        <?=form_hidden("gisto_detail[material_desc][$k]", $item_x[$i]['MAKTX']);?>
        </td>
        <!--dari sini -->
        <td>
        <?=$item_x[$i]['num'];?>
        <?=form_hidden("gisto_detail[num][$k]", $item_x[$i]['num']);?>
        </td>
        <!-- sampe siniii neehhh -->
		<td align="center">
<?php 
			if(!empty($error) && in_array("gisto_detail[gr_quantity][$i]", $error)) {
				echo form_input("gisto_detail[gr_quantity][$k]", number_format($data['gisto_detail']['gr_quantity'][$i], 2, '.', ''), 'class="error_number" size="8"');
			} else {
				echo form_hidden("gisto_detail[gr_quantity][$k]", $data['gisto_detail']['gr_quantity'][$i]);
                if($data['gisto_header']['status'] != '2')
    				echo form_input("gisto_detail[gr_quantity][$k]", number_format($data['gisto_detail']['gr_quantity'][$i], 2, '.', ''), 'class="input_number" size="8"');
                else
    				echo number_format($data['gisto_detail']['gr_quantity'][$i], 2, '.', ',');
			}
?></td>
		<td>
        <?php /* if (((strcmp($item_x[$i]['uom'],'KG')==0)||(strcmp($item_x[$i]['uom'],'G')==0))&&($data['gisto_header']['status'] != '2')):
            if(isset($_POST['button']['add'])||isset($_POST['button']['delete'])) { $item_x[$i]['uom'] = 'G'; } ?>
        <?php elseif (((strcmp($item_x[$i]['uom'],'L')==0)||(strcmp($item_x[$i]['uom'],'ML')==0))&&($data['gisto_header']['status'] != '2')):
           if(isset($_POST['button']['add'])||isset($_POST['button']['delete'])) { $item_x[$i]['uom'] = 'ML'; } ?>
        <?php endif; */?>
         <?=$item_x[$i]['uom'];?>
         <?=form_hidden("gisto_detail[uom][$k]", $item_x[$i]['uom']);?>
        </td>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("gisto_detail[material_no][$i]", $error)) {
			echo form_dropdown("gisto_detail[material_no][$k]", $item, $data['gisto_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select error_text" style="width:450px;" onChange="document.form1.submit();"');
		} else {
			echo form_dropdown("gisto_detail[material_no][$k]", $item, $data['gisto_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select input_text" style="width:450px;" onChange="document.form1.submit();"');
		}
		?>
		</td>
		...
		$it=$data['gisto_detail']['material_no'][$i];
		...
		if(!empty($error) && in_array("gisto_detail[num][$i]", $error)) {
			echo form_dropdown("gisto_detail[num][$k]", $item1, $data['gisto_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select error_text" style="width:450px;" ');
		} else {
		echo form_dropdown("gisto_detail[num][$k]", $item1  , $data['gisto_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select input_text" style="width:450px;"  ');  
		}
		}
		else
		{
		...
		<?php
		if(!empty($error) && in_array("gisto_detail[gr_quantity][$i]", $error)) {
			echo form_input("gisto_detail[gr_quantity][$k]", $data['gisto_detail']['gr_quantity'][$i], 'class="error_number" size="8"');
		} else {
			echo form_input("gisto_detail[gr_quantity][$k]", $data['gisto_detail']['gr_quantity'][$i], 'class="input_number" size="8"');
		}
		?>
		</td>
        <!-- dari sini -->
...
");
		...
		if(!empty($error) && in_array("gisto_detail[uom][$i]", $error)) {
			echo form_dropdown("gisto_detail[uom][$k]", $itemC, $data['gisto_detail']['uom'][$i], ' data-placeholder="Pilih UOM..." class="chosen-select error_text" style="width:10px;" ');
		} else {
		echo form_dropdown("gisto_detail[uom][$k]", $itemC  , $data['gisto_detail']['uom'][$i], ' data-placeholder="Pilih UOM..." class="chosen-select input_text" style="width:10px;"  ');  
		}
		}
	   ?>
        </td>
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	<tr>
    <td align="center"><?=anchor('gisto/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
