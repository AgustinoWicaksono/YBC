...
?>
  <tr>
	...
	<tr>
	  ...
	  $plant1=$grsto_header['SUPPL_PLANT'];
	  $cePL=mysql_fetch_array(mysql_query("SELECT TRANSIT,OUTLET_NAME1 FROM m_outlet WHERE OUTLET='$plant1' "));
    ...
    <td><?php if(!isset($data['grsto_header']['status']) || $data['grsto_header']['status'] == '1') : ?>
      ...
      <?=$i;?></td>
		...
		$id=$grsto_header['MBLNR'];
		  ...
		  mysql_connect("localhost",$user_mysql,$pass_mysql);
		mysql_select_db($db_mysql);
		$ta=mysql_query("SELECT id_gistonew_out_header FROM t_gistonew_out_header WHERE po_no = '$id'");
		$ra=mysql_fetch_array($ta);
		$gisto=$ra['id_gistonew_out_header'];
		$t=mysql_query("SELECT * FROM t_batch WHERE BaseEntry = '$gisto' AND ItemCode = '$material_no' AND BaseLinNum = '$item' ");
        ...
        <?=form_hidden("grsto_detail[item][$i]", $item);?>
		...
		<td align="center"><?=number_format($outstanding_qty, 3, '.', ',');?><?=form_hidden("grsto_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
        <td align="center"><?php
		if($data['grsto_header']['status'] != '2') {
			if(!empty($error) && in_array("grsto_detail[gr_quantity][$i]", $error)) {
				echo form_input("grsto_detail[gr_quantity][$i]", number_format($data['grsto_detail']['gr_quantity'][$i], 3, '.', ''), 'class="error_number" size="8"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grsto_detail[gr_quantity][$i]", number_format($gr_quantity[$i], 3, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grsto_detail[gr_quantity][$i]", number_format($data['grsto_detail']['gr_quantity'][$i], 3, '.', ''), 'class="input_number" size="8"');
            }
			}
		} else {
			echo number_format($data['grsto_detail']['gr_quantity'][$i], 3, '.', ',');
		}
		?></td>
		...
		if($data['grsto_header']['status'] != '2') {
			if(!empty($error) && in_array("grsto_detail[var][$i]", $error)) {
				echo form_input("grsto_detail[var][$i]", number_format($data['grsto_detail']['var'][$i], 3, '.', ''), 'class="error_number" size="4"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grsto_detail[var][$i]", number_format($var[$i], 3, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grsto_detail[var][$i]", number_format($data['grsto_detail']['var'][$i], 3, '.', ''), 'class="input_number" size="4"');
            }
			}
		} else {
			echo number_format($data['grsto_detail']['gr_quantity'][$i], 3, '.', ',');
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
        ...
        <?php endif; ?>
        <?php endif; ?><a href="#" onclick="window.open('<?=base_url();?>help/Good Receipt  Stock Transfer antar Plant.htm','popup','left=900 height=600,width=800,scrollbars=yes'); return(false);">
