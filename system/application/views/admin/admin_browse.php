...
<?=form_open('admin/browse_search', $form1);?>
	<tr>
  ...
  <?=form_close();?>
	...
	<tr>
		<td colspan="3" class="space">[<?=anchor('admin/input', $this->lang->line('admin_add'));?>]</td>
	</tr>
	<tr>
		<td>
...
<?php
				...
				echo anchor_image('admin/profile_edit/'.$admin['admin_id'], $this->lang->line('admin_profile_edit'), 'edit.png', 'height="20" width="20"');

				echo " ";
				echo anchor_image('admin/password_edit/'.$admin['admin_id'], $this->lang->line('admin_password_edit'), 'password.png', 'height="20" width="20"');
/*
			if( $this->l_auth->is_have_perm($perm_codes['edit']['group_perm']) ) {
				echo " ";
				echo anchor_image('admin/perm_group_edit/'.$admin['admin_id'], $this->lang->line('admin_perm_group_edit'), 'folder.png', 'height="20" width="20"');
			}
*/
			if( ($this->session->userdata['ADMIN']['admin_id'] != $admin['admin_id']) ) {

					echo " ";
//				echo anchor_image('#', $this->lang->line('admin_perm_group_edit'), 'delete.png', "height=\"20\" width=\"20\" onClick='confirm_admin_delete(\"".site_url('admin/delete/'.$admin['admin_id'])."\", \"".$admin['admin_realname']."\")')");
?>
<a href="#" onClick='confirm_admin_delete("<?=site_url('admin/delete/'.$admin['admin_id']);?>", "<?=$admin['admin_username'];?>")'><img src="<?=base_url();?>images/<?=$this->session->userdata('template');?>/<?=$this->session->userdata('lang_name');?>/delete.png" title="<?=$this->lang->line('delete');?>" height="20" width="20" /></a>
<?php
			}
?></td>
				</tr>
