...
?>
	...
	<tr>
		...
		<td class="column_input"><?=form_hidden('grnonpo_header[id_grnonpo_header]', $grnonpo_header['id_grnonpo_header']);?><strong><?=$grnonpo_header['id_grnonpo_header'];?></strong></td>
	</tr>
	<tr>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("grnonpo_detail[material_no][$i]", $error)) {
			echo form_dropdown("grnonpo_detail[material_no][$k]", $item, $data['grnonpo_detail']['material_no'][$i], 'data-placeholder="Pilih material..."  class="chosen-select error_text" style="width:450px;" onChange="document.form1.submit();" ');
		} else {
			echo form_dropdown("grnonpo_detail[material_no][$k]", $item, $data['grnonpo_detail']['material_no'][$i], 'data-placeholder="Pilih material..."  class="chosen-select input_text" style="width:450px;" onChange="document.form1.submit();" ');
		}
		?>
		</td>
		<td align="center">
		<?php
		$ot=$data['grnonpo_detail']['material_no'][$i];
		if($ot!='')
		{
			if(!empty($error) && in_array("grnonpo_detail[quantity][$i]", $error)) {
			echo form_input("grnonpo_detail[quantity][$k]", $data['grnonpo_detail']['quantity'][$i], 'class="error_number" size="8" autofocus=”autofocus”');
			} else {
			echo form_input("grnonpo_detail[quantity][$k]", $data['grnonpo_detail']['quantity'][$i], 'class="input_number" size="8" autofocus=”autofocus”');
			}
		}else{
		if(!empty($error) && in_array("grnonpo_detail[quantity][$i]", $error)) {
		...
		<td><?php
		$it=$data['grnonpo_detail']['material_no'][$i];
...
$count1=mysql_num_rows($ta);
		...
		if(!empty($error) && in_array("grnonpo_detail[uom][$i]", $error)) {
			echo form_dropdown("grnonpo_detail[uom][$k]",$itemC, $data['grnonpo_detail']['uom'][$i], ' data-placeholder="Pilih UOM..." class="chosen-select error_text " style="width:10px;" ');
		} else {
		echo form_dropdown("grnonpo_detail[uom][$k]" ,$itemC  , $data['grnonpo_detail']['uom'][$i], ' data-placeholder="Pilih UOM..." class="chosen-select input_text " style="width:10px;"  ');  
		}
		}
?></td>
		<td align="center">
