...
?>
		<td align="right" class="column_input"><script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
	    ...
	    <?php if($data['grpo_header']['status'] == '2') : ?>
...
<?php endif; ?>
       ...
       <td align="center"><?=number_format($outstanding_qty, 3, '.', ',');?><?=form_hidden("grpo_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
		...
		if($data['grpo_header']['status'] != '2') {
			...
			if(!empty($gr_quantity)) {
          	echo form_input("grpo_detail[gr_quantity][$i]", number_format($gr_quantity[$i], 3, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grpo_detail[gr_quantity][$i]", number_format($data['grpo_detail']['gr_quantity'][$i], 3, '.', ''), 'class="input_number" size="8"');
            }
			}
		} else {
			echo number_format($data['grpo_detail']['gr_quantity'][$i], 3, '.', ',');
		}
		?>        </td>
		<td><?=$uom;?></td>
