...
?>
<?=form_open('grpo/file_import_execute', $form1);?>
...
if($grpo_headers !== FALSE) :
	$i = 1;
	foreach ($grpo_headers as $grpo_header) :
?>
	<tr>
		...
		<td class="table_content_1"><?=$grpo_header['plant'];?></td>
		<td class="table_content_1"><?=$grpo_header['storage_location'];?></td>
		<td class="table_content_1">Unapproved</td>
	</tr>
<?php
		$j = 1;		
		foreach ($grpo_details[$i] as $grpo_detail[$i]) :
?>
	<tr>
		<td class="table_content_2"><?=$j;?></td>
		<td class="table_content_2"><?=$grpo_detail[$i]['material_no'];?></td>
		<td class="table_content_2"><?=$grpo_detail[$i]['material_desc'];?></td>
		<td class="table_content_2"><?=$grpo_detail[$i]['outstanding_qty'];?></td>
		<td class="table_content_2"><?=$grpo_detail[$i]['gr_quantity'];?></td>
		<td class="table_content_2"><?=$grpo_detail[$i]['uom'];?></td>
	</tr>
<?php
			$j++;
		endforeach;
