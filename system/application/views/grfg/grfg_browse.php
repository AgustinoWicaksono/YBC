...
<script language="javascript">
	function confirm_grfg_delete(url, id_grfg_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt FG Outlet  dengan ID "' + id_grfg_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<p>&nbsp;</p>
<?=form_open('grfg/browse_search', $form1);?>
...
<?=form_open('grfg/delete_multiple_confirm', $form2);?>
<table width="1000"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="160" class = "table_header_1" align="center"><strong>Goods Receipt FG. No<br /> <?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'], 'Z-A','panahatas.jpg');?> </strong></td>
...
if($data['grfg_headers'] !== FALSE) :
	$i = 1;
	foreach ($data['grfg_headers']->result_array() as $grfg_header):
		$grfg_header['status_string'] = ($grfg_header['status'] == '2') ? 'Approved' : 'Not Approved';
        if($this->m_grfg->grfg_is_data_cancelled($grfg_header['id_grfg_header']))
           $grfg_header['cancel_status'] = 'Y';
        else
           $grfg_header['cancel_status'] = 'N';
?>
...
<?php endif; ?>
    ...
    <td class=""><?=form_checkbox('id_grfg_header['.$i.']', $grfg_header['id_grfg_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('grfg/edit/'.$grfg_header['id_grfg_header'].'/'.$grfg_header['item_group_code'].'/1', 'Ubah dengan Item berdasarkan ID', 'edit.png', 'height="20" width="20"');?> <?=anchor_image('grfg/edit/'.$grfg_header['id_grfg_header'].'/'.$grfg_header['item_group_code'].'/2', 'Ubah dengan Item berdasarkan Nama', 'view.png', 'height="20" width="20"');?> <?php if($grfg_header['status'] == 1) : ?><a href="#" onClick='confirm_grfg_delete("<?=site_url('grfg/delete/'.$grfg_header['id_grfg_header']);?>", "<?=$grfg_header['id_grfg_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="table_content_1" nowrap="nowrap"><?=$grfg_header['plant'].'10';?><?=date("ymd", strtotime($grfg_header['posting_date']));?><?=sprintf("%06d", $grfg_header['id_grfg_plant']);?></td>
        <td class = ""><?=$grfg_header['grfg_no'];?></td>
	<td class = ""><?=date("d-m-Y", strtotime($grfg_header['posting_date']));?></td>
	<td><?=$grfg_header['status_string'];?></td>
	<td align="center"><?=$grfg_header['cancel_status'];?></td>
...
<?=form_close();?>
<?=form_open_multipart('grfg/file_import');?>
<table width="1000" align="center" class ="table_border_1">
  <tr>
    ...
    <td width="354"  colspan="3" class="space"><?=anchor('grfg/input', 'Add New Goods Receipt FG Outlet');?></td>
  </tr>
</table>
<?=form_close();?>
<table class="table-browse" width="100%" align="center">
