...
?>
  ...
  <tr>
    ...
    <td class="column_input"><strong>
      <?=form_hidden('gistonew_out_header[do_no]', $gistonew_out_header['VBELN']);?>
      <?=$gistonew_out_header['VBELN']; ?>
      </strong>
        <?php if($function == 'input2') echo ' '.anchor('gistonew_out/input', '<strong>Pilih ulang SR No dan Jenis Material</strong>');?></td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <tr>
    ...
    <?php
    $rq=mssql_fetch_array(mssql_query("SELECT ToWhsCode,OWHS.WhsName FROM OWTQ JOIN OWHS ON OWHS.WhsCode=OWTQ.ToWhsCode WHERE DOcEntry = '$gistonew_out_header[VBELN]'"));
	?>
    <td class="column_input"><?=$rq['ToWhsCode'].' - '.$rq['WhsName'];?>
    <?=form_hidden('gistonew_out_header[receiving_plant]', $rq['ToWhsCode']);?>    </td>
  </tr>
  <tr>
...
<?php endif; ?>
        ...
        <?=form_hidden("gistonew_out_detail[posnr][$k]", $item_x[$i]['POSNR']);?>
        <?php endif; ?><?=$k;?></td>
        ...
        <td>
        <?=$item_x[$i]['num'];?>
        <?=form_hidden("gistonew_out_detail[num][$k]", $item_x[$i]['num']);?>
        </td>
        <td> <?=$item_x[$i]['outstanding_qty'];?>
        <?=form_hidden("gistonew_out_detail[outstanding_qty][$k]", $item_x[$i]['outstanding_qty']);?> </td>
		<td align="center">
<?php
...
<?php endif; ?>
		...
		<td colspan="2" >
		<?php
		$te=mssql_query("SELECT WTQ1.ItemCode MATNR,Dscription MAKTX, (OpenQty - U_grqty_web) qty,LineNum POSNR  FROM OWTQ INNER JOIN WTQ1 ON
...
OWTQ.DocEntry = '$gistonew_out_header[VBELN]'
		...
		if(!empty($error) && in_array("gistonew_out_detail[num][$i]", $error)) {
			echo form_dropdown("gistonew_out_detail[num][$k]", $item2, $data['gistonew_out_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select error_text" style="width:450px;" ');
		} else {
		echo form_dropdown("gistonew_out_detail[num][$k]", $item2  , $data['gistonew_out_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select input_text" style="width:450px;"  ');  
		}
		}
		else
		{
		...
		{
		//echo $qty[$i];
		echo number_format($qty[$i], 3, '.', ',');?><?=form_hidden("gistonew_out_detail[outstanding_qty][$i]",$qty[$i]);
		}
		?>
		</td>
		<td align="center">
		...
		{
		if(!empty($error) && in_array("gistonew_out_detail[gr_quantity][$i]", $error)) {
			echo form_input("gistonew_out_detail[gr_quantity][$k]",$qty[$i], $data['gistonew_out_detail']['gr_quantity'][$i], 'class="error_number" size="8" ');
		} else {
			echo form_input("gistonew_out_detail[gr_quantity][$k]",$qty[$i], $data['gistonew_out_detail']['gr_quantity'][$i], 'class="input_number" size="8" ');
		}
		}
		else
		{
