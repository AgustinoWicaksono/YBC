...
?>
  ...
  <tr>
	...
	?>
      ...
      <?=$gistonew_dept_header['VBELN']; ?>
        ...
        <?php if($function == 'input2') echo ' '.anchor('gistonew_dept/input', '<strong>Pilih ulang SR No dan Jenis Material</strong>');
		}
		else
		{
  ...
  <!--tr>
    ...
    <td class="column_input"><?=form_hidden('gistonew_dept_header[posting_date]', $gistonew_dept_header['posting_date']);?>
        <strong>
          <?php if(!empty($gistonew_dept_header['posting_date'])) : ?>
          <?=$this->l_general->sqldate_to_date($gistonew_dept_header['posting_date']);?>
          <?php endif; ?>
        </strong></td>
  </tr-->
  <tr>
  ...
  <tr>
    ...
    <?=$item_group['item_group_name'];?>      <?=form_hidden("gistonew_dept_header[item_group_code]", $gistonew_dept_header['item_group_code']);?></td>
  </tr>
  <tr>
...
<?php endif; ?>
        ...
        <td>
        <?=$data['gistonew_dept_detail']['stock'][$i];?>
        <?=form_hidden("gistonew_dept_detail[stock][$k]",number_format($data['gistonew_dept_detail']['stock'][$i]), 2, '.', '');?>
        </td>
...
WHERE A.WhsCode ='$whs' AND A.ItemCode='$it' AND B.DistNumber='$batch'"));
		...
		echo $InWhs['Quantity'];?>
        <?=form_hidden("gistonew_dept_detail[stock][$k]", $InWhs['Quantity'],$data['gistonew_dept_detail']['stock'][$i]);?>
		
        </td>
        <td align="center">
		<?php
		$rowQty=mssql_fetch_array(mssql_query("SELECT OpenQty,LineNum FROM WTQ1 WHERE DocEntry='$gistonew_dept_header[VBELN]' AND ItemCode='$it'"));
		$qty=$rowQty['OpenQty'];
		$posnr=$rowQty['LineNum'];
		if ($it != '')
		{
		//echo $qty[$i];
		echo number_format($qty, 3, '.', ',');?><?=form_hidden("gistonew_dept_detail[outstanding_qty][$i]",$qty);
		}
		?>
