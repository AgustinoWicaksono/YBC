...
?>
	<tr>
		...
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gistonew_out_header']['gistonew_out_no'];?></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Goods Receipt Number </strong></td>
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gistonew_out_header']['po_no'];?></td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <?=form_hidden("gistonew_out_header[receiving_plant]", $receiving_plant['WERKS']);?>
        <?=form_hidden("gistonew_out_header[receiving_plant_name]", $receiving_plant['NAME1']);?>
        </td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Status </strong></td>
		<td class = "column_input"><strong><?=($function == 'input2') ? 'Not Approved' : $data['gistonew_out_header']['status_string'];?></strong></td>
	</tr>
	<tr>
        ...
        <?=$item_group['item_group_code'];?><?=form_hidden("gistonew_out_header[item_group_code]", $item_group['item_group_code']);?></td>
	</tr>
	<tr>
...
<?php
	if(empty($_POST) && $function == 'input2')
        ...
        if ((isset($_POST['button']['add']))||(count($_POST['delete'])>0))
    		...
    		$posting_date = date("d-m-Y",strtotime($this->l_general->str_datetime_to_date($data['gistonew_out_header']['posting_date'])));
        //date("d-m-Y", strtotime($this->l_general->strtodate($data['gistonew_out_header']['posting_date'])));
?>
		<td class="column_input"><?php if(($data['gistonew_out_header']['status'] !== '2')&&($this->l_auth->is_have_perm('auth_backdate'))) : ?>
		...
		</script><?php else: ?><?=$posting_date;?><?=form_hidden("gistonew_out_header[posting_date]", $posting_date);?><?php endif;?></td>
	</tr>
</table>
...
//	$i = 1; => already defined on FOR looping below
	...
	// for POST data
	if((!isset($_POST['delete']))&&(empty($gistonew_out_detail_tmp)))
		$count2 = $count;
	else
		$count2 = $count - 1;
...
<?php endif; ?>
        ...
        <?php endif; ?><?=$k;?></td>
		<td align="left"><?=form_hidden("gistonew_out_detail[material_no][$k]", $data['gistonew_out_detail']['material_no'][$i]);?><?=$data['gistonew_out_detail']['material_no'][$i];?></td>
		<td align="left"><?=$item_x[$i]['MAKTX'];?></td>
		<td align="center">
<?php
			if(!empty($error) && in_array("gistonew_out_detail[gr_quantity][$i]", $error)) {
				echo form_input("gistonew_out_detail[gr_quantity][$k]", $data['gistonew_out_detail']['gr_quantity'][$i], 'class="error_number" size="6"');
			} else {
                ...
                if($data['gistonew_out_header']['status'] != '2')
    				echo form_input("gistonew_out_detail[gr_quantity][$k]", $data['gistonew_out_detail']['gr_quantity'][$i], 'class="input_number" size="6"');
                else
    				echo $data['gistonew_out_detail']['gr_quantity'][$i];
			}
?></td>
		<td><?=$item_x[$i]['UNIT'];?></td>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("gistonew_out_detail[material_no][$i]", $error)) {
			echo form_dropdown("gistonew_out_detail[material_no][$k]", $item, $data['gistonew_out_detail']['material_no'][$i], 'class="error_text"');
		} else {
			echo form_dropdown("gistonew_out_detail[material_no][$k]", $item, $data['gistonew_out_detail']['material_no'][$i], 'class="input_text"');
		}
		?>
		</td>
		<td align="center">
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td align="center"><?php if($data['gistonew_out_header']['status'] == '2') : ?>
        ...
        <?php endif; ?>
        <?php endif; ?>
        <?=anchor('gistonew_out/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
