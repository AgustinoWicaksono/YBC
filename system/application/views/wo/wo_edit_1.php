...
?>
	<tr>
		...
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gisto_dept_header']['gisto_dept_no'];?></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Goods Receipt Number </strong></td>
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gisto_dept_header']['po_no'];?></td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <?=form_hidden("gisto_dept_header[receiving_plant]", $receiving_plant['WERKS']);?>
        <?=form_hidden("gisto_dept_header[receiving_plant_name]", $receiving_plant['NAME1']);?>
        </td>
	</tr>
	<tr>
...
//	$i = 1; => already defined on FOR looping below
	...
	// for POST data
	if((!isset($_POST['delete']))&&(empty($gisto_dept_detail_tmp)))
		$count2 = $count;
	else
		$count2 = $count - 1;
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if($data['gisto_dept_header']['status'] == '1' || ( ($data['gisto_dept_header']['status'] == '2') && !empty($data['gisto_dept_detail']['gr_quantity'][$i]) )) {
			$j++;
		}

		if(empty($_POST['delete'][$i])) {

			if($k == $count && !empty($error) && ( in_array("gisto_dept_detail[gr_quantity][$i]", $error) || in_array("gisto_dept_detail[material_no][$i]", $error) ))
				break;
...
<?php endif; ?>
        ...
        <?php endif; ?><?=$k;?></td>
		<td align="left"><?=form_hidden("gisto_dept_detail[material_no][$k]", $data['gisto_dept_detail']['material_no'][$i]);?><?=$data['gisto_dept_detail']['material_no'][$i];?></td>
		<td align="left"><?=$item_x[$i]['MAKTX'];?></td>
		<td align="center">
<?php
			if(!empty($error) && in_array("gisto_dept_detail[gr_quantity][$i]", $error)) {
				echo form_input("gisto_dept_detail[gr_quantity][$k]", $data['gisto_dept_detail']['gr_quantity'][$i], 'class="error_number" size="6"');
			} else {
				echo form_hidden("gisto_dept_detail[gr_quantity][$k]", $data['gisto_dept_detail']['gr_quantity'][$i]);
                if($data['gisto_dept_header']['status'] != '2')
    				echo form_input("gisto_dept_detail[gr_quantity][$k]", $data['gisto_dept_detail']['gr_quantity'][$i], 'class="input_number" size="6"');
                else
    				echo $data['gisto_dept_detail']['gr_quantity'][$i];
			}
?></td>
		<td><?=$item_x[$i]['UNIT'];?></td>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("gisto_dept_detail[material_no][$i]", $error)) {
			echo form_dropdown("gisto_dept_detail[material_no][$k]", $item, $data['gisto_dept_detail']['material_no'][$i], 'class="error_text"');
		} else {
			echo form_dropdown("gisto_dept_detail[material_no][$k]", $item, $data['gisto_dept_detail']['material_no'][$i], 'class="input_text"');
		}
		?>
		</td>
		<td align="center">
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
        ...
        <?php endif; ?>
        <?php endif; ?>
        <?=anchor('gisto_dept/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
