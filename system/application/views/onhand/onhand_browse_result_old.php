...
<table style="border-collapse:collapse;" width="2158" border="1" align="center" class="table">
  <tr>
    ...
    <td width="44" rowspan="3" align="center" bgcolor="#999999"><strong><span class="style5">Cutting In</span></strong></td>
    <td width="53" rowspan="3" align="center" bgcolor="#999999"><strong><span class="style5">Cutting Out</span></strong></td>
...
?>
  <tr>
    ...
    <td><span class="style5"><?=$r['BVolume'];?></span></td>
    <td><span class="style5">&nbsp;</span></td>
    <td><span class="style5"><?=$r['InvntryUom'];?></span></td>
    <td><span class="style5"><?php
    $item=$r['ItemCode'];
	$temp1=mssql_query("SELECT SUM(InQty) as  bgn FROM OIVL WHERE ItemCode='$item' AND LocCode='$plant1' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
...
;
    ...
    <td><span class="style5">
      <?php $temp6=mssql_query("SELECT SUM(Quantity) as qty_in FROM WTR1 WHERE ItemCode ='$item' AND WhsCode ='$plant' AND U_Retur <> 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120)");
	$r6=mssql_fetch_array($temp6);
	echo $r6['qty_in'];
	?>
    </span></td>
    <td><span class="style5"><?php
    $temp4=mssql_query("SELECT DocEntry FROM WTR1 WHERE ItemCode ='$item' AND WhsCode ='$plant1' AND U_Retur = 1  AND FromWhsCod NOT LIKE '%$plant2%' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120)");
	...
	?></span></td>
    <td><span class="style5"><?PHP $temp5=mssql_query("SELECT SUM(Quantity) as qty_in FROM WTR1 WHERE ItemCode ='$item' AND WhsCode ='$plant1' AND U_Retur = 1  AND FromWhsCod NOT LIKE '%$plant2%' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
	...
	echo $onhand;
	?></span></td>
    <td><span class="style5"><?PHP $temp7=mssql_query("SELECT SUM(Quantity) as cut_in FROM IGN1 WHERE ItemCode = '$item' AND WhsCode= 'plant1' AND U_BaseType=202 AND U_Cutting = 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
	$r7=mssql_fetch_array($temp7);
	
	echo $r7['cut_in'];
	?></span></td>
    <td><span class="style5"><?PHP $temp8=mssql_query("SELECT SUM(Quantity) as cut_out FROM IGE1 WHERE ItemCode = '$item' AND WhsCode= 'plant1' AND U_BaseType=202 AND U_Cutting = 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
	...
	?></span></td>
    <td><span class="style5"></span></td>
    <td><span class="style5"><span class="style5"><?PHP $temp9=mssql_query("SELECT DocEntry FROM WTQ1 WHERE ItemCode ='$item' AND WhsCode ='$plant'  AND FromWhsCod  LIKE '%$plant2%' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
	$r9=mssql_fetch_array($temp9);
	
	echo $r9['DocEntry'];
	?></span></span></td>
    <td><span class="style5">
      <?PHP $temp10=mssql_query("SELECT SUM(Quantity) as Qty_Cake FROM WTQ1 WHERE ItemCode ='$item' AND WhsCode ='$plant'  AND FromWhsCod  LIKE '%$plant2%' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120)");
    ...
    <td><span class="style5">&nbsp;</span></td>
    <td><span class="style5"><?php
    $temp11=mssql_query("SELECT DocEntry FROM WTR1 WHERE ItemCode ='$item' AND FromWhsCod ='$plant' AND U_Retur <> 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120)");
while ($r11=mssql_fetch_array($temp11))
    ...
    <td><span class="style5">
      <?php
    $temp12=mssql_query("SELECT SUM(Quantity)  as Qty_Out FROM WTR1 WHERE ItemCode ='$item' AND WhsCode ='$plant' AND (U_Retur <> 1 OR U_Retur IS NULL) AND FromWhsCod  LIKE '%$plant2%' AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
$r12=mssql_fetch_array($temp12);
	echo $r12['Qty_Out'];
	?>
    </span></td>
    <td><span class="style5"><?PHP $temp13=mssql_query("SELECT DocEntry FROM IGE1 WHERE ItemCode = '$item' AND WhsCode= 'plant1'  AND U_Waste = 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
	...
	?></span></td>
    <td><span class="style5"><?PHP $temp14=mssql_query("SELECT SUM(Quantity) as qty_wast FROM IGE1 WHERE ItemCode = '$item' AND WhsCode= 'plant1' AND U_Waste = 1 AND DocDate <=  convert(datetime, '$date_to2', 120) AND DocDate >=  convert(datetime, '$date_from', 120) ");
...
$r17=mssql_fetch_array($temp17);
	...
	?></span></td>
    <td><span class="style5"><?php
	$variance=$ending_stock-$r15['count'];
    echo $variance;
	?></span></td>
    <td><span class="style5">&nbsp;</span></td>
    <td><span class="style5">&nbsp;</span></td>
