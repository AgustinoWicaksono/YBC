...
<?php /* <div style="color:#ffffff;font-style:italic;font-size:90%;">Jika data tidak lengkap, harap tunggu sinkronisasi berikutnya, yaitu 2 jam dari waktu di atas.</div> */ ?>
	...
	<?php if(!empty($data['do_no'])) : ?>
        ...
        <?php if(!empty($_POST)) : if(!empty($data['do_no'])) : echo anchor('grpofg/input', '<strong>Pilih ulang Delivery Order</strong>'); endif; endif;?>
        </td>
	<?php else : ?>
		<?php if (count($do_no)>0) { ?>
		<td class="column_input">Anda memiliki <b style="color:#aa1122;"><?php echo count($do_no)-1; ?> nomor DO</b> yang harus <b><u>segera</u></b> diterima dan disetujui.<hr /><?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="input_text" onChange="document.form1.submit();"');?></td>
		<?php } else { ?>
		<td class="column_input"><b style="color:#aa1122;">Saat ini tidak ada nomor DO untuk PO STO.</b><br /><i>Silahkan hubungi bagian inventory di SX untuk informasi lebih lanjut, jika Anda merasa ada kesalahan sistem.</i><hr />Silahkan email: <b>sap.helpdesk@ybc.co.id</b></td>
		<?php } ?>
	<?php endif; ?>
	</tr>
<?php if(isset($_POST['do_no'])) : ?>
	...
	<tr>
		...
		<td class="column_input"><?php if(!empty($data['grpofg_header']['DELIV_DATE'])) : ?><?=$this->l_general->sqldate_to_date($data['grpofg_header']['DELIV_DATE']);?><?php endif; ?></td>
	</tr>
	<tr>
