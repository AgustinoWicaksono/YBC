...
<script language="javascript">
	function confirm_prodstaff_delete(url, id_prodstaff_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Productivity Staff dengan ID "' + id_prodstaff_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<table width="600"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
	...
	<td width="70" class = "table_header_1"><strong>Action</strong> <strong></strong></td>
...
<?php endif; ?>
	...
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('prodstaff/edit/'.$prodstaff_header['id_prodstaff_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?>
    <?php if($prodstaff_header['status'] == 1) : ?><a href="#" onClick='confirm_prodstaff_delete("<?=site_url('prodstaff/delete/'.$prodstaff_header['id_prodstaff_header']);?>", "<?=$prodstaff_header['id_prodstaff_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="" nowrap="nowrap"><?=$prodstaff_header['plant'].'13';?><?=date("ymd", strtotime($prodstaff_header['posting_date']));?><?=sprintf("%06d", $prodstaff_header['id_prodstaff_plant']);?></td>
...
<?=form_close();?>
<?=form_open_multipart('prodstaff/file_import');?>
<table width="600" align="center" class ="table_border_1">
  <tr>
