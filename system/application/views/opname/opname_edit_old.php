...
?>
		<td class="column_input">
        <?php if($data['opname_header']['status'] != '2') : ?>
        <?=form_input('opname_header[delivery_date]', $delivery_date, 'class="input_text" size="10"');?> <script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
...
?>
     <td class="column_input">
     <?php if(($data['opname_header']['status'] != '2')&&($this->l_auth->is_have_perm('auth_backdate'))) : ?>
		<?=form_input('opname_header[created_date]', $created_date, 'class="input_text" size="10"');?> <script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
    ...
    <?php else : ?>
    <?=$created_date;?><?=form_hidden("opname_header[created_date]", $created_date);?>
    <?php endif;?></td>
	</tr>
    <?php 
...
//	$i = 1; => already defined on FOR looping below
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if($data['opname_header']['status'] == '1' || ( ($data['opname_header']['status'] == '2') && !empty($data['opname_detail']['requirement_qty'][$i]) )) {
			$j++;
		}
		...
		if(empty($_POST['delete'][$i])) {
            ...
            if (!empty($data['opname_detail']['material_desc'][$i])) {
              ...
              $item_x[$i]['uom'] = $data['opname_detail']['uom'][$i];
            }
?>
  <?php if($i % 2) : ?>
...
<?php
			if(!empty($error) && in_array("opname_detail[requirement_qty][$i]", $error)) {
				echo form_input("opname_detail[requirement_qty][$k]", number_format($data['opname_detail']['requirement_qty'][$i], 3, '.', ''), 'class="error_number" size="8"');
			} else {
                ...
                echo form_input("opname_detail[requirement_qty][$k]", number_format($data['opname_detail']['requirement_qty'][$i], 3, '.', ''), 'class="input_number" size="8"');
			}
?></td>
<!--td align="center">
<?php
...
?></td-->
        <td>
        <?=$data['opname_detail']['uom'][$i];?>
        <?=form_hidden("opname_detail[uom][$k]", $data['opname_detail']['uom'][$i]);?>
        </td>
...
<?php endif; ?>
		...
		<?php
		/*if(!empty($error) && in_array("opname_detail[opnameice][$i]", $error)) {
			echo form_input("opname_detail[opnameice][$k]", $data['opname_detail']['opnameice'][$i], 'class="error_number" size="8"');
		} else {
			echo form_input("opname_detail[opnameice][$k]", $data['opname_detail']['opnameice'][$i], 'class="input_number" size="8"');
		}*/
		?>
		</td-->
        <td>
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
        ...
        <?=form_submit($this->config->item('button_save'), $this->lang->line('button_save'));?>
            <?php if ($this->l_auth->is_have_perm('auth_apopnameove')) : ?>
            <?=form_submit($this->config->item('button_apopnameove'), $this->lang->line('button_apopnameove'));?>
            <?php endif; ?>
        <?php endif; ?>
	...
	<tr>
    <td align="center"><?=anchor('opname/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
