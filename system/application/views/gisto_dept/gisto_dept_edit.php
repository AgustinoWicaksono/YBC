...
?>
	<tr>
		...
		<td class="column_input"><?=form_hidden('gisto_dept_header[id_gisto_dept_header]', $gisto_dept_header['id_gisto_dept_header']);?><strong><?=$gisto_dept_header['id_gisto_dept_header'];?></strong></td>
	</tr>
	<!--tr>
	...
	<tr>
		<td width="512" class="table_field_1"><strong>Retur Number </strong></td>
		<td width="516" class="column_input"><?=empty($data['gisto_dept_header']['gisto_dept_no']) ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gisto_dept_header']['gisto_dept_no'];?></td>
	</tr>
	<tr>
	...
	<tr>
		<td class="table_field_1"><strong>Retur  Out to Outlet</strong> </td>
        ...
        <?=form_hidden("gisto_dept_header[receiving_plant_name]", $receiving_plant['plant_name']);?>        </td>
	</tr>
	<tr>
...
?>
	...
	<tr>
	  <td colspan="2" align="right" class="table_field_1"><?php if($data['gisto_dept_header']['status'] == '2') : ?>
...
<?php endif; ?>
        ...
        <!--dari sini -->
        <!--td>
        <?php /*$data['gisto_dept_detail']['num'][$i];?>
        <?=form_hidden("gisto_dept_detail[num][$k]", $data['gisto_dept_detail']['num'][$i]);*/?>
        </td>
        <!-- sampe siniii neehhh -->
        <td>
        <?=$data['gisto_dept_detail']['stock'][$i];?>
        <?=form_hidden("gisto_dept_detail[stock][$k]", $data['gisto_dept_detail']['stock'][$i]);?>
        </td>
		<td align="center">
<?php 
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("gisto_dept_detail[num][$i]", $error)) {
			echo form_dropdown("gisto_dept_detail[num][$k]", $item1, $data['gisto_dept_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select error_text" style="width:450px;" onChange="document.form1.submit();"');
		} else {
		echo form_dropdown("gisto_dept_detail[num][$k]", $item1  , $data['gisto_dept_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select input_text" style="width:450px;" onChange="document.form1.submit();" ');  
		}
		}
		else
		{
		...
		$batch=$data['gisto_dept_detail']['num'][$i];
		if ($batch=='')
		{
		...
		$InWhs=sqlsrv_fetch_array(sqlsrv_query("SELECT A.Quantity FROM OBTQ A
...
WHERE A.WhsCode ='$whs' AND A.ItemCode='$it' AND B.DistNumber='$batch'"));
        ...
        <?=form_hidden("gisto_dept_detail[stock][$k]", substr($InWhs['Quantity'],0,-4),$data['gisto_dept_detail']['stock'][$i]);?>
...
", $mysqlcon);
	   ...
	   ?>
        ...
        <td align="center">
		<?php
		if(!empty($error) && in_array("gisto_dept_detail[reason][$i]", $error)) {
			echo form_input("gisto_dept_detail[reason][$k]", $data['gisto_dept_detail']['reason'][$i], 'class="error_text" size="24"');
		} else {
			echo form_input("gisto_dept_detail[reason][$k]", $data['gisto_dept_detail']['reason'][$i], 'class="input_text" size="24"');
		}
		?>
		</td>
	</tr>
