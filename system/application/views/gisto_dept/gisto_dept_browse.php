...
<script language="javascript">
	function confirm_gisto_dept_delete(url, id_gisto_dept_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Retur  dengan ID "' + id_gisto_dept_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<table width="1200"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="150" class = "table_header_1" align="center"><strong>Retur Out No<br /> <?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'], 'Z-A','panahatas.jpg');?> </strong></td>
	...
	<td width="150" class="table_header_1" align="center" >Retur Out to Outlet<br />
	...
	<td width="60" class ="table_header_1" align="center"><strong>Log</strong></td>
  </tr>
  <?php
...
<?php endif; ?>
	...
	<td><?=$gisto_dept_header['status_string'];?></td>
		<?php
			$id=$gisto_dept_header['id_user_input'];
			$r=mysql_fetch_array(mysql_query("SELECT admin_realname FROM d_admin WHERE admin_id = '$id'"));
			$name=$r['admin_realname'];
		?>
		<td ><?php echo $name;?></td>
		<td ><?php if ($gisto_dept_header['status']== 2){
		  $id=$gisto_dept_header['id_user_approved'];
		  $r=mysql_fetch_array(mysql_query("SELECT admin_realname FROM d_admin WHERE admin_id = '$id'"));
		  $name=$r['admin_realname'];
		  echo $name; }
          else {echo "-";}?></td>
	
	<td><?=$gisto_dept_header['lastmodified'];?></td>
	 <td align="center"><?php $log=$gisto_dept_header['back'];
		$po=$gisto_dept_header['gisto_dept_no'];
		if ($log==0 && $po !='' && $po !='C')
		{
			echo "Integrated";
...
<?=form_close();?>
<?=form_open_multipart('gisto_dept/file_import');?>
<table width="1200" align="center" class ="table_border_1">
  <tr>
    <td width="220" >&nbsp;</td>
    <td width="330" >&nbsp;</td>
