...
<?=form_open('employee/browse_search', $form1);?>
  <tr>
    ...
    <td width="399">Tanggal Akhir Kontrak From  <?=form_input('date_from', $data['date_from'], 'class="input_text" size="10"');?> <script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
...
<table width="1330" border="0" align="center">
    ...
    <th width="40" class="table_header_1">&nbsp;</td>
		...
		<th width="180" class ="table_header_1">Saldo Cuti</th>
		<th width="180" class ="table_header_1">Saldo PH</th>
  </tr>
...
if($data['employees'] !== FALSE) :
	...
	foreach ($data['employees'] as $employee):
		...
		$one_month_int = 30 * 24 * 60 * 60;
		$one_month_limit_int = time() + $one_month_int;

		$tanggal_akhir_int = strtotime($employee['tanggal_akhir']);


?>
<?php if( ($employee['status_kerja'] != 'PERMANENT') AND ($tanggal_akhir_int < time()) ) : ?>
  <tr class="table_row_red">
<?php elseif( ($employee['status_kerja'] != 'PERMANENT') AND ($one_month_limit_int >= $tanggal_akhir_int) ) : ?>
  <tr class="table_row_yellow">
<?php elseif($i % 2) : ?>
  <tr class="table_row_1">
<?php else : ?>
  <tr class="table_row_2">
<?php endif; ?>
    <td class=""><?=form_checkbox('employee_id['.$i.']', $employee['employee_id'], FALSE);?></td>
		<td align="center" nowrap="nowrap"><?=anchor_image('employee/edit/'.$employee['employee_id'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <!--<?=anchor_image('employee/fp/'.$employee['nik'], 'Buat Fingerprint ID untuk cabang lain', 'arrow_right.png', 'height="20" width="20"');?>//--> <?php if($employee['status'] == 1) : ?><a href="#" onClick='confirm_employee_delete("<?=site_url('employee/delete/'.$employee['employee_id']);?>", "<?=$employee['employee_id'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
		<td><?=$employee['nik'];?></td>
		<td><?=$employee['nama'];?></td>
		...
		<?php if (intval($employee['fingerprint_id'])==0) { ?>
		<td class="grdF">Belum Terdaftar</td>
		<?php } else { ?>
		<td align="center"><?=$employee['fingerprint_id'];?></td>
		<?php } ?>
		<td><?=$employee['status_kerja'];?></td>
		<td><?php if($employee['status_kerja'] != 'PERMANENT') echo $employee['tanggal_akhir'];?></td>
		<td><?=$employee['current_saldo_cuti'];?></td>
		<td><?=$employee['saldo_ph'];?></td>
  </tr>
  <?php
		$i++;
	endforeach;
endif;
?><tr><td class="table_content_1" colspan="9"><?=form_hidden('count', --$i);?>
