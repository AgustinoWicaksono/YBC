...
<?php
$dates = count($data);
//print_r($this->session->userdata['ADMIN']['plant_type_id']);
...
<?php
	...
	//echo 'userdata'.$this->session->userdata['ADMIN']['plant'];
			if($j % 2) {
?>
...
<?php
					...
					if(!empty($data[$i][$j])) {
						...
						if($k == 5 || $k == 9 || $k == 12 || $k == 14 || $k == 16 || $k == 7 || $k == 11 || $k == 13 || $k == 15) { // for number
?>
...
?>
			<td align="right"><strong><?=$subtotal[$i][$k];?></strong></td>
<?php
					} else {
?>
			<td>&nbsp;</td>
...
?>
			<td align="right"><strong><?=$total[$k];?></strong></td>
<?php
						} else {
?>
			<td>&nbsp;</td>
