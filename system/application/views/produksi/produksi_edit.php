...
//$i = 0; //=> already defined on FOR looping below
	...
	/*echo "INSERT INTO m_mpaket_detail_temp SELECT a.id_mpaket_detail,a.id_mpaket_header,a.id_mpaket_h_detail,a.material_
		no,a.material_desc,a.quantity,a.uom,'$kd_plant',$user FROM m_mpaket_detail a 
		JOIN m_mpaket_header b  on a.id_mpaket_header=b.id_mpaket_header
...
<?php
		...
		$it=$r['material_no'];
		$sqlbom=sqlsrv_fetch_array(sqlsrv_query($b,"SELECT U_CanEditQty as CanEditQty FROM ITT1 
													WHERE Code='$it' AND Father='$kode_paket'"));
		$caneditqty= $sqlbom['CanEditQty'];

		$ctl=mysql_num_rows(mysql_query($tp,$mysqlcon));
		if ($ctl > 0)
		{
			if(!empty($error) && in_array("produksi_detail[qty][$i]", $error)) {
				echo form_input("produksi_detail[qty][$k]", number_format($qty, 4, '.', ''), 'class="error_number" size="8" disabled="TRUE"');
				
			} else {
				echo form_hidden("produksi_detail[qty][$k]", $qty);
				if(($data['produksi_header']['status'] != '2')&&($caneditqty=='Y'))
...
?>
	<tr>
		...
		if(!empty($error) && in_array("produksi_detail[material_no][$i]", $error)) {
			echo form_dropdown("produksi_detail[material_no][$k]", $item, $data['produksi_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select error_text" style="width:450px;" disabled="TRUE" onChange="document.form1.submit();"');
		} else {
			echo form_dropdown("produksi_detail[material_no][$k]", $item, $data['produksi_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select input_text" style="width:450px;" disabled="TRUE" onChange="document.form1.submit();"');
		}
		?>
		</td>
