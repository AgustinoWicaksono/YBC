...
//	$i = 1; => already defined on FOR looping below
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if($data['twts_header']['status'] == '1' || ( ($data['twts_header']['status'] == '2') && !empty($data['twts_detail']['quantity'][$i]) )) {
			$j++;
		}
...
<?php
        ...
        <?php
            if (empty($data['twts_detail']['quantity_gr'][$i]))
              $qty_gr = $data['twts_detail']['quantity'][$i]*$item_x_gr[$i]['quantity'];
            else
              $qty_gr = $data['twts_detail']['quantity_gr'][$i];
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td align="center"><?php if($data['twts_header']['status'] == '2') : ?>
