...
<script language="javascript">
	function confirm_tssck_delete(url, id_tssck_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Transfer Selisih Stock ke CK  dengan ID "' + id_tssck_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<p>&nbsp;</p>
<?=form_open('tssck/browse_search', $form1);?>
<table width="1060" align="center" class ="table_border_1">
  <tr>
    <td width="520" ><strong>Search by</strong>
...
<?=form_open('tssck/delete_multiple_confirm', $form2);?>
<table width="1150"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="154" class = "table_header_1" align="center" ><strong>Delivery Order No<br /><?=anchor_image($sort_link['gy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['gz'],'Z-A', 'panahatas.jpg');?></strong></td>
	<td width="116" class="table_header_1" align="center" >Posting Date<br /><?=anchor_image($sort_link['ey'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['ez'],'Z-A', 'panahatas.jpg');?></td>
	<td width="200" class="table_header_1" align="center" >Receiving Plant<br /><?=anchor_image($sort_link['dy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['dz'],'Z-A', 'panahatas.jpg');?></td>
...
if($data['tssck_headers'] !== FALSE) :
	$i = 1;
	foreach ($data['tssck_headers']->result_array() as $tssck_header):
		$tssck_header['status_string'] = ($tssck_header['status'] == '2') ? 'Approved' : 'Not Approved';
        if($this->m_tssck->tssck_is_data_cancelled($tssck_header['id_tssck_header']))
           $tssck_header['cancel_status'] = 'Y';
        else
           $tssck_header['cancel_status'] = 'N';
?>
...
<?php endif; ?>
    ...
    <td class=""><?=form_checkbox('id_tssck_header['.$i.']', $tssck_header['id_tssck_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('tssck/edit/'.$tssck_header['id_tssck_header'].'/'.$tssck_header['receiving_plant'].'/'.$tssck_header['do_no'].'/1', 'Ubah dengan Item berdasarkan ID', 'edit.png', 'height="20" width="20"');?> <?=anchor_image('tssck/edit/'.$tssck_header['id_tssck_header'].'/'.$tssck_header['receiving_plant'].'/'.$tssck_header['do_no'].'/2', 'Ubah dengan Item berdasarkan Nama', 'view.png', 'height="20" width="20"');?> <?php if($tssck_header['status'] == 1) : ?><a href="#" onClick='confirm_tssck_delete("<?=site_url('tssck/delete/'.$tssck_header['id_tssck_header']);?>", "<?=$tssck_header['id_tssck_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="table_content_1" nowrap="nowrap"><?=$tssck_header['plant'].'11';?><?=date("ymd", strtotime($tssck_header['posting_date']));?><?=sprintf("%06d", $tssck_header['id_tssck_plant']);?></td>
    <td class =""><?=$tssck_header['tssck_no'];?></td>
    <td class = ""><?=$tssck_header['po_no'];?></td>
    <td class = ""><?=$tssck_header['do_no'];?></td>
	<td class = ""><?=date("d-m-Y", strtotime($tssck_header['posting_date']));?></td>
	<td class = ""><?=$tssck_header['receiving_plant'];?>-<?=$tssck_header['receiving_plant_name'];?></td>
	<td><?=$tssck_header['status_string'];?></td>
	<td align="center"><?=$tssck_header['cancel_status'];?></td>
...
<?=form_close();?>
<?=form_open_multipart('tssck/file_import');?>
<table width="1150" align="center" class ="table_border_1">
  <tr>
    ...
    <td width="354"  colspan="3" class="space"><?=anchor('tssck/input', 'Add New Transfer Selisih Stock ke CK');?></td>
  </tr>
</table>
<?=form_close();?>
<table class="table-browse" width="100%" align="center">
