...
?>
	<tr>
		...
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['tssck_header']['tssck_no'];?></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Goods Receipt Number </strong></td>
		<td class="column_input"><?=($function == 'input2') ? '<em>(Auto Number after Posting to SAP)</em>' : $data['tssck_header']['po_no'];?></td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <?=form_hidden("tssck_header[receiving_plant]", $receiving_plant['WERKS']);?>
        <?=form_hidden("tssck_header[receiving_plant_name]", $receiving_plant['NAME1']);?>
        </td>
	</tr>
	<tr>
	...
	<tr>
        ...
        <?=$item_group['item_group_code'];?><?=form_hidden("tssck_header[item_group_code]", $item_group['item_group_code']);?></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Delivery Order Number</strong></td>
        <td class = "column_input">
		<?php
		if(!empty($error) && in_array("tssck_header[do_no]", $error)) {
			echo form_input("tssck_header[do_no]", $data['tssck_header']['do_no'], 'class="error_text" size="10"');
		} else {
			echo form_input("tssck_header[do_no]", $data['tssck_header']['do_no'], 'class="input_text" size="10"');
		}
		?>
        </td>
	</tr>
...
//	$i = 1; => already defined on FOR looping below
	...
	// for POST data
	if((!isset($_POST['delete']))&&(empty($tssck_detail_tmp)))
		$count2 = $count;
	else
		$count2 = $count - 1;
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if(empty($_POST['delete'][$i])) {
			...
			if($k == $count && !empty($error) && ( in_array("tssck_detail[gr_quantity][$i]", $error) ||
                                                   ...
                                                   in_array("tssck_header[do_no]", $error) ))
				break;

			$item_x[$i] = $this->m_general->sap_item_select_by_item_code($data['tssck_detail']['material_no'][$i]);
?>
  <?php if($i % 2) : ?>
...
<?php endif; ?>
        ...
        <?php endif; ?><?=$k;?></td>
		<td align="left"><?=form_hidden("tssck_detail[material_no][$k]", $data['tssck_detail']['material_no'][$i]);?><?=$data['tssck_detail']['material_no'][$i];?></td>
		<td align="left"><?=$item_x[$i]['MAKTX'];?></td>
		<td align="center">
<?php
			if(!empty($error) && in_array("tssck_detail[gr_quantity][$i]", $error)) {
				echo form_input("tssck_detail[gr_quantity][$k]", $data['tssck_detail']['gr_quantity'][$i], 'class="error_number" size="6"');
			} else {
				echo form_hidden("tssck_detail[gr_quantity][$k]", $data['tssck_detail']['gr_quantity'][$i]);
                if($data['tssck_header']['status'] == '2')
    				echo $data['tssck_detail']['gr_quantity'][$i];
                else
    				echo form_input("tssck_detail[gr_quantity][$k]", $data['tssck_detail']['gr_quantity'][$i], 'class="input_number" size="6"');
			}
?></td>
		<td><?=$item_x[$i]['UNIT'];?></td>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("tssck_detail[material_no][$i]", $error)) {
			echo form_dropdown("tssck_detail[material_no][$k]", $item, $data['tssck_detail']['material_no'][$i], 'class="error_text"');
		} else {
			echo form_dropdown("tssck_detail[material_no][$k]", $item, $data['tssck_detail']['material_no'][$i], 'class="input_text"');
		}
		?>
		</td>
		<td align="center">
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
        ...
        <?php else: ?> <?=form_submit($this->config->item('button_save'), $this->lang->line('button_save'));?> <?=form_submit($this->config->item('button_approve'), $this->lang->line('button_approve'));?> <?php endif; ?> <?=anchor('tssck/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
