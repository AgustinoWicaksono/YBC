...
?>
	<tr>
		...
		<td class="column_input"><?=empty($data['gitcc_header']['gitcc_no']) ? '<em>(Auto Number after Posting to SAP)</em>' : $data['gitcc_header']['gitcc_no'];?></td>
	</tr>
	<tr>
...
//	$i = 1; => already defined on FOR looping below
	...
	// for POST data
	if((!isset($_POST['delete']))&&(empty($gitcc_detail_tmp))&&(empty($error)))
		$count2 = $count;
	else
		$count2 = $count - 1;
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if(empty($_POST['delete'][$i])) {
            ...
            if (!empty($data['gitcc_detail']['material_desc'][$i])) {
              $item_x[$i]['MAKTX'] = $data['gitcc_detail']['material_desc'][$i];
            }
            if (!empty($data['gitcc_detail']['uom'][$i])) {
              $item_x[$i]['UNIT'] = $data['gitcc_detail']['uom'][$i];
            }
?>
  <?php if($i % 2) : ?>
...
<?php endif; ?>
        ...
        <?=form_hidden("gitcc_detail[material_desc][$k]", $item_x[$i]['MAKTX']);?>
        </td>
		<td align="center">
<?php
			if(!empty($error) && in_array("gitcc_detail[quantity][$i]", $error)) {
				echo form_input("gitcc_detail[quantity][$k]", number_format($data['gitcc_detail']['quantity'][$i], 2, '.', ''), 'class="error_number" size="8"');
			} else {
				echo form_hidden("gitcc_detail[quantity][$k]", $data['gitcc_detail']['quantity'][$i]);
                if($data['gitcc_header']['status'] != '2')
   				  echo form_input("gitcc_detail[quantity][$k]", number_format($data['gitcc_detail']['quantity'][$i], 2, '.', ''), 'class="input_number" size="8"');
                else
   				  echo number_format($data['gitcc_detail']['quantity'][$i], 2, '.', ',');
			}
?></td>
		<td>
        <?php if (((strcmp($item_x[$i]['UNIT'],'KG')==0)||(strcmp($item_x[$i]['UNIT'],'G')==0))&&($data['gitcc_header']['status'] != '2')):
           if(isset($_POST['button']['add'])||isset($_POST['button']['delete'])) { $item_x[$i]['UNIT'] = 'G'; } ?>
        <?php elseif (((strcmp($item_x[$i]['UNIT'],'L')==0)||(strcmp($item_x[$i]['UNIT'],'ML')==0))&&($data['gitcc_header']['status'] != '2')):
           if(isset($_POST['button']['add'])||isset($_POST['button']['delete'])) { $item_x[$i]['UNIT'] = 'ML'; } ?>
        <?php endif; ?>
         <?=$item_x[$i]['UNIT'];?>
         <?=form_hidden("gitcc_detail[uom][$k]", $item_x[$i]['UNIT']);?>
        </td>
...
<?php endif; ?>
		...
		<?php
		if(!empty($error) && in_array("gitcc_detail[quantity][$i]", $error)) {
			echo form_input("gitcc_detail[quantity][$k]", $data['gitcc_detail']['quantity'][$i], 'class="error_number" size="8"');
		} else {
			echo form_input("gitcc_detail[quantity][$k]", $data['gitcc_detail']['quantity'][$i], 'class="input_number" size="8"');
		}
		?>
		</td>
		<td>&nbsp;</td>
...
<table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	<tr>
    <td align="center"><?=anchor('gitcc/browse', $this->lang->line('button_back'));?></td>
	</tr>
</table>
<?=form_close();?>
