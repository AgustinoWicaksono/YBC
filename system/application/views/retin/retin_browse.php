...
<script language="javascript">
	function confirm_retin_delete(url, id_retin_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO dengan ID "' + id_retin_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<p>&nbsp;</p>
<?=form_open('retin/browse_search', $form1);?>
...
<?=form_open('retin/delete_multiple_confirm', $form2);?>
<table width="1000"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="160" class = "table_header_1" align="center"><strong>Retur Out No <br /><?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'], 'Z-A','panahatas.jpg');?> </strong></td>
    <td width="159" class = "table_header_1" align="center" ><strong>Retur In  No<br /><?=anchor_image($sort_link['cy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['cz'],'Z-A', 'panahatas.jpg');?></strong></td>
...
if($data['retin_headers'] !== FALSE) :
	$i = 1;
	foreach ($data['retin_headers']->result_array() as $retin_header):
		$retin_header['status_string'] = ($retin_header['status'] == '2') ? 'Approved' : 'Not Approved';
        if($this->m_retin->retin_is_data_cancelled($retin_header['id_retin_header']))
           $retin_header['cancel_status'] = 'Y';
        else
           $retin_header['cancel_status'] = 'N';
?>
...
<?php endif; ?>
   <td class=""><?=form_checkbox('id_retin_header['.$i.']', $retin_header['id_retin_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('retin/printpdf/'.$retin_header['id_retin_header'], 'Print Transaksi', 'print.png', 'height="20" width="20"');?><?=anchor_image('retin/edit/'.$retin_header['id_retin_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <?php if($retin_header['status'] == 1) : ?><a href="#" onClick='confirm_retin_delete("<?=site_url('retin/delete/'.$retin_header['id_retin_header']);?>", "<?=$retin_header['id_retin_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<!--td class="table_content_1" nowrap="nowrap"><?//=$retin_header['plant'].'02';?><?//=date("ymd", strtotime($retin_header['posting_date']));?><?//=sprintf("%06d", $retin_header['id_retin_plant']);?></td-->
    <td class =""><?=$retin_header['id_retin_header'];?></td>
    <td class =""><?=$retin_header['do_no'];?></td>
    <td class = ""><?=$retin_header['retin_no'];?></td>
	<!--td><?php if(!empty($retin_header['delivery_date'])) : ?><?=$this->l_general->sqldate_to_date($retin_header['delivery_date']);?><?php endif; ?></td-->
	<td class = ""><?=date("d-m-Y", strtotime($retin_header['posting_date']));?></td>
	<td><?=$retin_header['status_string'];?></td>
	<td align="center"><?=$retin_header['cancel_status'];?></td>
...
<?=form_close();?>
<?=form_open_multipart('retin/file_import');?>
<table width="1000" align="center" class ="table_border_1">
  <tr>
    <td width="220">&nbsp;</td>
    <td width="330" >&nbsp;</td>
    <td width="354" colspan="3" class="space"><?=anchor('retin/input', 'Add New Retur In');?></td>
  </tr>
</table>
<?=form_close();?>
<table class="table-browse" width="100%" align="center">
