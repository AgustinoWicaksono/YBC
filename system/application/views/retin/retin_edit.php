...
<?php /* <div style="color:#ffffff;font-style:italic;font-size:90%;">Jika data tidak lengkap, harap tunggu sinkronisasi berikutnya, yaitu 2 jam dari waktu di atas.</div> */ ?>    </td>
	...
	<tr>
		...
		<td class="column_input"><?=form_hidden('retin_header[id_retin_header]', $retin_header['id_retin_header']);?><strong><?=$retin_header['id_retin_header'];?></strong></td>
	</tr>
  <tr>
	...
	<tr>
		<td class="table_field_1"><strong>Retur From</strong></td>
		<td class="column_input">
        <strong><?php 
		$from=mysql_fetch_array(mysql_query("SELECT plant,m_outlet.OUTLET_NAME1 FROM t_gisto_dept_header  JOIN m_outlet ON m_outlet.OUTLET=t_gisto_dept_header.plant WHERE gisto_dept_no=$retin_header[VBELN]"));
		echo $from['plant'].' - '.$from['OUTLET_NAME1'] ;
		 ?></strong></td>
	</tr>
	<tr>
		<td class="table_field_1"><strong>Outlet To</strong></td>
...
?>
	...
	<tr>
	  <td colspan="2" align="right" class="table_field_1"><?php if($data['retin_header']['status'] == '2') : ?>
...
//        }
	...
	/*	  $c=mssql_connect("SVR-S-2012-64","sa","abc123?");
        ...
        <!--td><?php /*$retin_detail['material_docno_cancellation'][$i];?><?=form_hidden("retin_detail[material_docno_cancellation][$i]", $retin_detail['material_docno_cancellation'][$i]);*/?></td-->
        <?php
		if ($function=='edit' && $data['retin_header']['status'] != '2')
		{?>
			<td align="right"><?=number_format($outstanding_qty, 2, '.', ',');?><?=form_hidden("retin_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
		<?php }
		else
		{ ?>
			<td align="right"><?=number_format($outstanding_qty, 2, '.', ',');?><?=form_hidden("retin_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
		...
		if($data['retin_header']['status'] != '2') {
			if(!empty($error) && in_array("retin_detail[gr_quantity][$i]", $error)) {
				echo form_input("retin_detail[gr_quantity][$i]", number_format($data['retin_detail']['gr_quantity'][$i], 2, '.', ''), 'class="error_number" size="8"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("retin_detail[gr_quantity][$i]", number_format($gr_quantity[$i], 2, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("retin_detail[gr_quantity][$i]", number_format($data['retin_detail']['gr_quantity'][$i], 2, '.', ''), 'class="input_number" size="8"');
            }
			}
		} else {
			echo number_format($data['retin_detail']['gr_quantity'][$i], 2, '.', ',');
		}
		?></td>
		<td><?=$uom;?></td>
    <?php if(($data['retin_header']['status'] == '2')&&($this->l_auth->is_have_perm('auth_cancel'))): ?>
