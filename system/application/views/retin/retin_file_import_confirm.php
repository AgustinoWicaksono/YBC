...
?>
<?=form_open('retin/file_import_execute', $form1);?>
...
if($retin_headers !== FALSE) :
	$i = 1;
	foreach ($retin_headers as $retin_header) :
?>
	<tr>
        <td class="table_content_1"><?=$retin_header['retin_no'];?></td>
		<td class="table_content_1"><?=$retin_header['do_no'];?></td>     
			<td class="table_content_1"><?=$retin_header['plant'];?></td>
		<td class="table_content_1"><?=$retin_header['storage_location'];?></td>
...
<?php
		$j = 1;
		foreach ($retin_details[$i] as $retin_detail[$i]) :
?>
	<tr>
		<td class="table_row_2"><?=$j;?></td>
		<td class="table_row_2"><?=$retin_detail[$i]['material_no'];?></td>
		<td class="table_row_2"><?=$retin_detail[$i]['material_desc'];?></td>
		<td class="table_row_2"><?=$retin_detail[$i]['outstanding_qty'];?></td>
		<td class="table_row_2"><?=$retin_detail[$i]['gr_quantity'];?></td>
		<td class="table_row_2"><?=$retin_detail[$i]['uom'];?></td>
	</tr>
<?php
			$j++;
		endforeach;
