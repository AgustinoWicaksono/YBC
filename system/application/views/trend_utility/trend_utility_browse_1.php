...
<script language="javascript">
	function confirm_trend_utility_delete(url, id_trend_utility) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO from Vendor dengan ID "' + id_trend_utility_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<?=form_open('trend_utility/browse_search', $form1);?>
  <tr>
    <td width="490" ><strong>Search by</strong>
      ...
      <td width="399">Posting Date Form  <?=form_input('date_from', $data['date_from'], 'class="input_text" size="10"');?> <script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
...
<?php endif; ?>
  <td class=""><?=form_checkbox('id_trend_utility_header['.$i.']', $trend_utility['id_trend_utility_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('trend_utility/edit/'.$trend_utility['id_trend_utility_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <?php if($trend_utility['status'] == 1) : ?><a href="#" onClick='confirm_trend_utility_delete("<?=site_url('trend_utility/delete/'.$trend_utility['id_trend_utility_header']);?>", "<?=$trend_utility['id_trend_utility_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="" nowrap="nowrap"><?=$trend_utility['plant'];?><?=date("ymd", strtotime($trend_utility['posting_date']));?><?=sprintf("%06d", $trend_utility['id_trend_utility_plant']);?></td>
...
?><tr><td class="table_content_1" colspan="11"><?=form_checkbox('select_all', 'select_all', FALSE, '<b>'.$this->lang->line('select_all').'</b>', 'onclick="selectAll()"');?><?=form_hidden('count', --$i);?><?=form_submit('button[delete]', 'Hapus data yang dipilih');?></td>
</tr>
<?=form_close();?>
