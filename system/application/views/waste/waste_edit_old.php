...
?>
    ...
    <tr>
		<td class="table_field_1"><strong>No. Berita Acara</strong></td>
		<td class="column_input"><?php
			if(!empty($error) && in_array("waste_header[no_acara][1]", $error)) {
			echo form_input("waste_header[no_acara][1]", $data['waste_header']['no_acara'][1], 'class="error_text" size="24"');
		} else {
			echo form_input("waste_header[no_acara][1]", $data['waste_header']['no_acara'][1], 'class="input_text" size="24"');
		}
		?></td>
	</tr>
</table>
...
//	$i = 1; => already defined on FOR looping below
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if(empty($_POST['delete'][$i])) {
			...
			if($k == $count && !empty($error) && ( in_array("waste_detail[quantity][$i]", $error) || in_array("waste_detail[material_no][$i]", $error) || in_array("waste_detail[reason_name][$i]", $error ) || in_array("waste_header[no_acara][1]", $error ) || in_array("delete[$i]", $error ) ))
				break;
...
<?php endif; ?>
		...
		$line=$data['waste_detail']['id_waste_h_detail'][$k];
		$entry=$data['waste_header']['id_waste_header'];
		$to="SELECT BatchNum FROM t_batch where BaseEntry='$entry' and ItemCode = '$item' and BaseLinNum = '$line'";
		//echo $to;
		$t=mysql_query($to); 
		$ra=mysql_fetch_array($t);
			
             //$ra['BatchNum'] = $data['waste_detail']['num'][$i]; 
		?>
        <?= $item_x[$i]['BatchNum'];?>
...
<?php endif; ?>
		...
		if(!empty($error) && in_array("waste_detail[material_no][$i]", $error)) {
			echo form_dropdown("waste_detail[material_no][$k]", $item, $data['waste_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select error_text" style="width:450px;" onChange="document.form1.submit();" ');
		} else {
			echo form_dropdown("waste_detail[material_no][$k]", $item, $data['waste_detail']['material_no'][$i], ' data-placeholder="Pilih material..." class="chosen-select input_text" style="width:450px;" onChange="document.form1.submit();" ');
		}
		?>
		</td>
         <?php
		...
		$t=mysql_query("SELECT DistNumber BatchNum FROM m_batch where ItemCode='$it' AND Whs ='$whs'"); 
		$count=mysql_num_rows($t);
		$item2= "Tidak ada Batch Number";
		while($r=mysql_fetch_array($t))
		{
			$item1[$r['BatchNum']] = $r['BatchNum'];
		} ?>
        <td >
		<?php
		if(!empty($error) && in_array("waste_detail[num][$i]", $error)) {
			echo form_dropdown("waste_detail[num][$k]", $item1, $data['waste_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select error_text" style="width:450px;" ');
		} else {
			echo form_dropdown("waste_detail[num][$k]", $item1, $data['waste_detail']['num'][$i], ' data-placeholder="Pilih Batch Number..." class="chosen-select input_text" style="width:450px;" ');
		}
		?>
		</td>
		<td align="center">
...
<?php if($data['waste_header']['status'] != '2') : ?>
        ...
        <a href="#" onclick="window.open('<?=base_url();?>help/Waste Material.htm','popup','left=900 height=600,width=800,scrollbars=yes'); return(false);">
