...
<script language="javascript">
	function confirm_waste_delete(url, id_waste_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO from Vendor dengan ID "' + id_waste_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<table width="1000"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="160" class = "table_header_1" align="center"><strong>Material Doc No<br /> <?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'], 'Z-A','panahatas.jpg');?> </strong></td>
...
<?php endif; ?>
	...
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('waste/edit/'.$waste_header['id_waste_header'].'/'.$waste_header['item_group_code'].'/1', 'Ubah dengan Item berdasarkan ID', 'edit.png', 'height="20" width="20"');?> <?=anchor_image('waste/edit/'.$waste_header['id_waste_header'].'/'.$waste_header['item_group_code'].'/2', 'Ubah dengan Item berdasarkan Nama', 'view.png', 'height="20" width="20"');?> <?php if($waste_header['status'] == 1) : ?><a href="#" onClick='confirm_waste_delete("<?=site_url('waste/delete/'.$waste_header['id_waste_header']);?>", "<?=$waste_header['id_waste_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="table_content_1" nowrap="nowrap"><?=$waste_header['plant'];?><?=date("ymd", strtotime($waste_header['posting_date']));?><?=sprintf("%06d", $waste_header['id_waste_plant']);?></td>
...
<table width="1000" align="center" class ="table_border_1">
  <tr>
	...
	<?=form_close();?>
    <td width="354"  colspan="3" class="space"><?=anchor('waste/input', 'Add New Waste Material');?></td>
  </tr>
</table>
<table class="table-browse" width="100%" align="center">
<tr>
