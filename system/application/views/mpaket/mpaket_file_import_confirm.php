...
?>
<?=form_open('mpaket/file_import_execute', $form1);?>
...
if($mpaket_headers !== FALSE) :
	$i = 1;
	foreach ($mpaket_headers as $mpaket_header) :
?>
	<tr>
        ...
        <td class="table_content_1"><?=number_format($mpaket_header['quantity_paket'], 2, '.', '').' '.$mpaket_header['uom_paket'];?></td>
	   	<td class="table_content_1"><?=$mpaket_header['plant'];?></td>
	</tr>
<?php
		$j = 1;
		foreach ($mpaket_details[$i] as $mpaket_detail[$i]) :
?>
	<tr>
		<td class="table_row_2"><?=$j;?></td>
		<td class="table_row_2"><?=$mpaket_detail[$i]['material_no'];?></td>
		<td class="table_row_2"><?=$mpaket_detail[$i]['material_desc'];?></td>
		<td class="table_row_2"><?=$mpaket_detail[$i]['quantity'];?></td>
		<td class="table_row_2"><?=$mpaket_detail[$i]['uom'];?></td>
	</tr>
<?php
			$j++;
		endforeach;
