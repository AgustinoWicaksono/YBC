...
<script language="javascript">
	function confirm_grpodlv_delete(url, id_grpodlv_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO from Vendor dengan ID "' + id_grpodlv_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<?php endif; ?>
	...
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('grpodlv/edit/'.$grpodlv_header['id_grpodlv_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <?php if($grpodlv_header['status'] == 1) : ?><a href="#" onClick='confirm_grpodlv_delete("<?=site_url('grpodlv/delete/'.$grpodlv_header['id_grpodlv_header']);?>", "<?=$grpodlv_header['id_grpodlv_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="table_content_1" nowrap="nowrap"><?=$grpodlv_header['plant'];?><?=date("ymd", strtotime($grpodlv_header['posting_date']));?><?=sprintf("%06d", $grpodlv_header['id_grpodlv_plant']);?></td>
	...
	<td align="center"><?=$grpodlv_header['cancel_status'];?></td>
