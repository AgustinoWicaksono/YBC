...
<?php /* <div style="color:#ffffff;font-style:italic;font-size:90%;">Jika data tidak lengkap, harap tunggu sinkronisasi berikutnya, yaitu 2 jam dari waktu di atas.</div> */ ?>
	...
	if(!empty($data['do_no'])) : ?>
		<td class="column_input"><?=form_hidden('do_no', $data['do_no']);?><?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="input_text" disabled="disabled"');?>
        <?php if(!empty($_POST)) : if(!empty($data['do_no'])) : echo anchor('grpodlv/input', '<strong>Pilih ulang Transfer Slip Number</strong>'); endif; endif;?>
        </td>
	<?php else : ?>
		<?php if (count($do_no)>0) { ?>
		<td class="column_input">Anda memiliki <b style="color:#aa1122;"><?php echo count($do_no)-1; ?> Transfer Slip Number</b> yang harus <b><u>segera</u></b> diterima dan disetujui.<hr /><?=form_dropdown('do_no', $do_no, $data['do_no'], 'class="chosen-select input_text" data-placeholder="Pilih Nomor Receipt..." onChange="document.form1.submit();"');?></td>
		<?php } else { ?>
		<td class="column_input"><b style="color:#aa1122;">Saat ini tidak ada nomor Transfer Slip Number .</b><br /><i>Silahkan hubungi bagian inventory di SX untuk informasi lebih lanjut, jika Anda merasa ada kesalahan sistem.</i><hr />Silahkan email: <b>sap.helpdesk@ybc.co.id</b></td>
		<?php } ?>
	<?php endif; ?>
	</tr>
<?php if(isset($_POST['do_no'])) : ?>
	...
	<tr> 
		...
		<td class="column_input"><?php if(!empty($data['grpodlv_header']['DELIV_DATE'])) : ?><?=$this->l_general->sqldate_to_date($data['grpodlv_header']['DELIV_DATE']);?><?php endif; ?></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="column_input"><?=form_hidden('to_plant', $data['grpodlv_header']['PLANT']);?><?=$this->session->userdata['ADMIN']['storage_location'].' - '.$this->session->userdata['ADMIN']['storage_location_name']." (".$data['grpodlv_header']['PLANT'].")";?></td>
	</tr>
	<tr>
