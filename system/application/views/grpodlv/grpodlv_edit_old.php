...
//        }
        ...
        <td width="177"><?=$grpodlv_detail['num'][$i];?><?=form_hidden("grpodlv_detail[num][$i]", $grpodlv_detail['num'][$i]);?></td>
		<td align="right"><?=number_format($outstanding_qty, 3, '.', ',');?><?=form_hidden("grpodlv_detail[outstanding_qty][$i]",$outstanding_qty);?></td>
        <td align="center"><?php
		if($data['grpodlv_header']['status'] != '2') {
			if(!empty($error) && in_array("grpodlv_detail[gr_quantity][$i]", $error)) {
				echo form_input("grpodlv_detail[gr_quantity][$i]", number_format($data['grpodlv_detail']['gr_quantity'][$i], 3, '.', ''), 'class="error_number" size="8"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grpodlv_detail[gr_quantity][$i]", number_format($gr_quantity[$i], 3, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grpodlv_detail[gr_quantity][$i]", number_format($data['grpodlv_detail']['gr_quantity'][$i], 3, '.', ''), 'class="input_number" size="8"');
            }
			}
		} else {
			echo number_format($data['grpodlv_detail']['gr_quantity'][$i], 3, '.', ',');
		}
		?></td>
		<td><?=$uom;?></td>
        <td><?php
		...
		if($data['grpodlv_header']['status'] != '2') {
			if(!empty($error) && in_array("grpodlv_detail[var][$i]", $error)) {
				echo form_input("grpodlv_detail[var][$i]", number_format($data['grpodlv_detail']['var'][$i], 3, '.', ''), 'class="error_number" size="4"');
			} else {
			if(!empty($gr_quantity)) {
          	echo form_input("grpodlv_detail[var][$i]", number_format($var[$i], 3, '.', ''), 'class="input_number" size="8"');
			} else {
          	echo form_input("grpodlv_detail[var][$i]", number_format($data['grpodlv_detail']['var'][$i], 3, '.', ''), 'class="input_number" size="4"');
            }
			}
		} else {
			echo number_format($data['grpodlv_detail']['gr_quantity'][$i], 3, '.', ',');
