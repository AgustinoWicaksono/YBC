...
//	$i = 1; => already defined on FOR looping below
	...
	for($i = 1; $i <= $count2; $i++) {
		...
		if($data['nonstdstock_header']['status'] == '1' || ( ($data['nonstdstock_header']['status'] == '2') && !empty($data['nonstdstock_detail']['requirement_qty'][$i]))) {
			$j++;
		}

		if(empty($_POST['delete'][$i])) {
			if($k == $count && !empty($error) && ( in_array("nonstdstock_detail[requirement_qty][$i]", $error) || in_array("nonstdstock_detail[material_no][$i]", $error) || in_array("nonstdstock_detail[delivery_date][$i]", $error)) )
				break;
...
?>
        ...
        <?=$data['nonstdstock_detail']['ok_cancel'][$i];?><?php endif; ?>        </td>
        <?php endif; ?>
  </tr>
<?php
        $k++;
