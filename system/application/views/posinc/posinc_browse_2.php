...
<table width="1000"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="116" class="table_header_1" align="center" >Posting Date<br /><?=anchor_image($sort_link['cy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['cz'],'Z-A', 'panahatas.jpg');?></td>
	 <td width="116" class="table_header_1" align="center" >Total Remintance<br /><?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'],'Z-A', 'panahatas.jpg');?></td>
...
<?php endif; ?>
	...
	<td class="" width="180" nowrap="nowrap"><?=$posinc_header['plant'];?><?=date("ymd", strtotime($posinc_header['posting_date']));?><?=sprintf("%06d", $posinc_header['id_posinc_plant']);?></td>
...
?><tr><td class="table_content_1" colspan="9">
      <?=form_checkbox('select_all', 'select_all', FALSE, '<b>'.$this->lang->line('select_all').'</b>', 'onclick="selectAll()"');?><?=form_hidden('count', --$i);?><?=form_submit('button[delete]', 'Hapus data yang dipilih');?>
      <?php
      $id_posinc_outlet = $this->m_posinc->posinc_header_select_id_by_posting_date();
      if($id_posinc_outlet == FALSE) : ?>
          <?=anchor('posinc/input', 'Add New');?>
      <?php else : ?>
          <?=anchor('posinc/input_transaksi_baru_error', 'Add New');?>
      <?php endif; ?>
      </td>
</tr>
<?=form_close();?>
