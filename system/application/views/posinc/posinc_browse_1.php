...
<script language="javascript">
	function confirm_posinc_delete(url, id_posinc_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Goods Receipt PO from Vendor dengan ID "' + id_posinc_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<?=form_open('posinc/browse_search', $form1);?>
  <tr>
    <td width="430"><strong>Posting Date Form  <?=form_input('date_from', $data['date_from'], 'class="input_text" size="10"');?> <script language="JavaScript">
					new tcal ({
						// form name
						'formname': 'form1',
						// input name
...
<table width="600"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    ...
    <td width="116" class="table_header_1" align="center" ><strong>Posting Date<br /><?=anchor_image($sort_link['cy'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['cz'],'Z-A', 'panahatas.jpg');?></td>
	 <td width="116" class="table_header_1" align="center" ><strong>Total Remintance<br /><?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'],'Z-A', 'panahatas.jpg');?></td>
...
<?php endif; ?>
	...
	<td class="" align="left" nowrap="nowrap"><?=anchor_image('posinc/edit/'.$posinc_header['id_posinc_header'], $this->lang->line('edit'), 'edit.png', 'height="20" width="20"');?> <?php if($posinc_header['status'] == 1) : ?><a href="#" onClick='confirm_posinc_delete("<?=site_url('posinc/delete/'.$posinc_header['id_posinc_header']);?>", "<?=$posinc_header['id_posinc_header'];?>")'><img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a><?php endif; ?></td>
	<td class="" nowrap="nowrap"><?=$posinc_header['plant'];?><?=date("ymd", strtotime($posinc_header['posting_date']));?><?=sprintf("%06d", $posinc_header['id_posinc_plant']);?></td> 
    <td class = ""><?=date("d-m-Y", strtotime($posinc_header['posting_date']));?></td>
	<td><?=$posinc_header['total_remintance'];?></td>
	<td><?=$posinc_header['status_string'];?></td>
...
<table width="600" align="center" class ="table_border_1">
  <tr>
	...
	<?=form_close();?>
    <td width="300"  colspan="3" class="space"><?=anchor('posinc/input', 'Add New');?></td>
  </tr>
</table>
<table class="table-browse" width="100%" align="center">
<tr>
