...
$re=mysql_fetch_array(mysql_query($temp_cake, $mysqlcon));
$trans_plant=$re['cake'];
...
{
$grup_kode=$grup_item;
$swbQuery = "SELECT c.itemcode,c.ItemName,c.U_Small,c.BVolume BVolume,c.InvntryUom,d.MinStock,d.MaxStock, 
c.onhand, 
...
from 
oitm c,OITW d
where d.WhsCode = '$plant1' 
and c.ItemCode=d.ItemCode 
GROUP BY c.ItemCode,d.WhsCode,c.ItemName,c.BVolume,c.U_Small,c.InvntryUom,d.MinStock,d.MaxStock";
//sum(a.inqty-a.outqty)
//oivl a , 
//and a.LocCode = d.WhsCode 
//and a.DocDate between convert(datetime, '$date_from', 120) AND convert(datetime, '$date_to', 120)
//and LEFT(d.LocCode,2)!='T.'
...
$tmp=sqlsrv_query($c,"SELECT c.itemcode,c.ItemName,c.U_Small,c.BVolume BVolume,c.InvntryUom,d.MinStock,d.MaxStock, 
c.onhand, 
(select sum(b.inqty-b.outqty) from oivl b where C.itemcode=b.ItemCode and b.LocCode=d.WhsCode and b.DocDate < convert(datetime, '$date_from', 120) ) stock_awal, 
(select sum(b.inqty) from oivl b where C.itemcode=b.ItemCode and b.LocCode=d.WhsCode and b.DocDate <= convert(datetime, '$date_to', 120) AND b.DocDate >= convert(datetime, '$date_from', 120) ) inqty, 
(select sum(b.OutQty) from oivl b where C.itemcode=b.ItemCode and b.LocCode=d.WhsCode and b.DocDate <= convert(datetime, '$date_to', 120) AND b.DocDate >= convert(datetime, '$date_from', 120) ) outqty, 
(select sum(inqty-outqty) from oivl b where C.itemcode=b.ItemCode and b.LocCode=d.WhsCode and b.DocDate<= convert(datetime, '$date_to', 120) ) stock_akhir 
from oitm c INNER JOIN OITW d ON c.ItemCode=d.ItemCode 
inner join oitb e on e.itmsgrpcod = c.itmsgrpcod
where d.WhsCode = '$plant1' 
and LEFT(d.WhsCode,2)!='T.'
and ((itmsGrpNam = '$item_group_code') or (itmsGrpNam = '==All=='))
GROUP BY c.ItemCode,d.WhsCode,c.ItemName,c.BVolume,c.U_Small,c.InvntryUom,d.MinStock,d.MaxStock,c.onhand");
?>
...
<a href="<?= site_url('pastry/browse_result_excel/'.$date_from.'/'.$item_group_code.'/'.$WhsCode.'/'.$date_to)?>">Cetak</a>
