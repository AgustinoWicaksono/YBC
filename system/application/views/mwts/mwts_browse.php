...
<script language="javascript">
	function confirm_mwts_delete(url, id_mwts_header) {
		var m = confirm('Apakah Anda setuju untuk menghapus data Master Konversi Item Whole ke Slice  dengan ID "' + id_mwts_header + '" dan data item-item barang di dalamnya? Data yang sudah dihapus tidak bisa dimunculkan kembali.');
		if(m) {
			location.href=url;
		}
	}
...
<p>&nbsp;</p>
<?=form_open('mwts/browse_search', $form1);?>
<table width="800" align="center" class ="table_border_1">
  <tr>
    ...
    <a href="#" onclick="window.open('<?=base_url();?>help/Master Konversi Item Whole ke Slice.htm','popup','left=900 height=600,width=800,scrollbars=yes'); return(false);">
...
<?=form_open('mwts/delete_multiple_confirm', $form2);?>
<table width="800"  align="center" class ="table_border_1">
  <tr bgcolor="#999999">
    <td width="10" class="table_header_1">&nbsp;</td>
	<td width="30" class = "table_header_1"><strong>Action</strong></td>
    <td width="150" class = "table_header_1" align="center"><strong>Whole Item No<br /> <?=anchor_image($sort_link['ay'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['az'], 'Z-A','panahatas.jpg');?> </strong></td>
    <td width="250" class="table_header_1" align="center" ><strong>Whole Item Description<br /><?=anchor_image($sort_link['by'],'A-Z', 'panahbawah.jpg');?><?=anchor_image($sort_link['bz'],'Z-A', 'panahatas.jpg');?></strong></td>
  </tr>
  <?php
if($data['mwts_headers'] !== FALSE) :
	$i = 1;
	foreach ($data['mwts_headers']->result_array() as $mwts_header):
?>
...
<?php endif; ?>
    ...
    <td class=""><?=form_checkbox('id_mwts_header['.$i.']', $mwts_header['id_mwts_header'], FALSE);?></td>
	<td class="" align="left" nowrap="nowrap">
    <?=anchor_image('mwts/edit/'.$mwts_header['id_mwts_header'].'/1', 'Ubah dengan Item berdasarkan ID', 'edit.png', 'height="20" width="20"');?>
    <?=anchor_image('mwts/edit/'.$mwts_header['id_mwts_header'].'/2', 'Ubah dengan Item berdasarkan Nama', 'view.png', 'height="20" width="20"');?>
    <a href="#" onClick='confirm_mwts_delete("<?=site_url('mwts/delete/'.$mwts_header['id_mwts_header']);?>", "<?=$mwts_header['id_mwts_header'];?>")'>
    <img src="<?=base_url().'images/'.$this->session->userdata('template').'/'.$this->session->userdata('lang_name').'/';?>delete.png" height="20" width="20" /></a>
    </td>
    <td class = ""><?=$mwts_header['kode_whi'];?></td>
    <td class = ""><?=$mwts_header['nama_whi'];?></td>
  </tr>
  <?php
		$i++;
	endforeach;
...
<?=form_close();?>
<?=form_open_multipart('mwts/file_import');?>
<table width="800" align="center" class ="table_border_1">
  <tr>
    ...
    <td width="354"  colspan="3" class="space"><?=anchor('mwts/input', 'Add New Whole Item');?></td>
  </tr>
</table>
<?=form_close();?>
<table class="table-browse" width="100%" align="center">
