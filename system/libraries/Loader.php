...
// Edited by LuckyGuy354
// NGga jadi diubah, tetap pakai yang asli yang di atas
			// Load all other libraries
/*
			foreach ($autoload['libraries'] as $key => $item)
			{
				if ($key){
					$this->library($item, $key);
//					$this->library($item,null,$key); => to change to another library name
				} else {
					$this->library($item);
				}
			}
